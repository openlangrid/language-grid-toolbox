...
var AddPathWorkspaceCollaborativeTranslation = Class.create();
AddPathWorkspaceCollaborativeTranslation.prototype = {
	MainPanel:null,
	...
	_showPanel: function(setting,listArea) {
		...
		var pathPanel = new TransPathPanel();
	...
	_makeEditSettingArea:function(){
		{
			var default_dic_btn = new Element('a')
				.update('<img src="'+Const.Images.DefaultDict+'" id="img-def-dict"> '+Const.Label.EditDefaultDict)
				.observe('click', this._onDefaultDictionaryButtonClicked.bind(this));
	
			var dics_count = new Element('span', {id: "def-dic-count"});
			
			var default_dic_area = new Element('div', {id: 'div-edit-default-area'})
				.insert(default_dic_btn)
				.insert(dics_count);
		}

		{
			var add_path_btn = new Element('a')
				.addClassName('btn btn-addpath')
				.update('<img src="'+Const.Images.AddPath+'"> '+Const.Label.AddPath)
				.observe('click', this._onAddPathButtonClicked.bind(this));
			
			var add_path_area = new Element('div', {id: 'div-add-path-area'})
				.insert(add_path_btn);
		}
			
		var setting_area = new Element('div',{id: 'div-setting-area'})
			.insert(default_dic_area)
			.insert(add_path_area);
		
		var baseElm = $(this.MainPanel);
		baseElm.insert(setting_area);

		baseElm.insert(
			new Element('div', {id: 'new-path-area'})
				.addClassName('div-path-add-area').hide()
		);
		
		baseElm.insert(
			new Element('div', {id: 'top-message-area'})
				.addClassName('div-message-box').hide()
		);
	...
	_makePageTopArea:function (){
		var hlink = new Element('a')
			.addClassName('inner-link')
			.update("<img src='./images/icon/icn_pagetop.gif' />"+Const.Label.ReturnTop)
			.observe('click',function(){
					Element.scrollTo($('settings'));
			});
		
		var pagetop_area = new Element('div')
			.addClassName('div-pagetop-area')
			.insert(hlink);
		
		$(this.MainPanel).insert(pagetop_area);
	},
	...
	_loadSetting: function() {
		...
		new Ajax.Request('./ajax/load-trans-setting.php', {
		//new Ajax.Request('./ajax/dummy.php', {
			...
			onSuccess: function(httpObj) {
				try {
					...
					if($('list-area')){ $('list-area').remove();}
					var list_area = new Element('div', {id: 'list-area'});
					$A(controller._langridServiceInformations['_currentSettings'])
						.each(function(setting, index){
	...
	onDictionarySaveHandler:function(){
		...
		var hash = $H(postObj).toQueryString();
		new Ajax.Request('./ajax/save-trans-dict.php', {
			method: 'post',
			parameters: hash,
			controller: controller,
			onSuccess: function(httpObj) {
