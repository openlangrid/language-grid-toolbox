...
//		return $result;
		...
		debugLog('## sources count ##'.count($sources));
		debugLog('## sourceKeys count ##'.count($sourceKeys));
		$targets = $this->doTranslate($sources);
		debugLog('## targets count ##'.count($targets));
		foreach ($sourceKeys as $i => $key) {
			try {
			$result[$key]->target = @$targets[$i];
			} catch (Exception $e) {
//				debugLog('i='.$i.', key='.$key);
			}	
		}
