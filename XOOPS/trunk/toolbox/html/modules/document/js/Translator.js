...
var Translator = Class.create( {
	...
	translate : function() {
		...
		voiceManager.setLanguage(this.targetLang);
	},
	...
	doTranslate : function(threadId, sourceLang, targetLang, allSentences) {
		...
		var type;
		var lite = $('lite-checkbox');
		var rich = $('rich-checkbox');
		if (lite.checked && rich.checked)
			type = 'dual';
		else if (lite.checked)
			type = 'lite';
		else if (rich.checked)
			type = 'rich';
		else 
			type = 'normal';

		Event.observe(lite, 'change', function() {
			this.clearCache();
		}.bind(this));

		Event.observe(rich, 'change', function() {
			this.clearCache();
		}.bind(this));
		...
		new Ajax.Request(
				...
				, {
					...
					, onSuccess : function(response){
						if (Process.isCanceled(pid)) return;

						try {
							try {
								var result = response.responseText.evalJSON()[0];
							} catch(e) {
								// invalid resp	onse. server error occurred? 
								for(var i = 0; i < sentences.length; i++) {
							...
							} else if (result.status.toLowerCase() == 'warning') {
								throw {message : this.WARNING, contents : response.responseText};
							}
							...
							if (e.message == this.WARNING) {
  								this.translateEditor.callRaiseWarning(e);
							} else {
								this.translateEditor.callRaiseError(e);
							}
						}
	...
	isTranslationComplete : function() {
		...
		// MARK
//		if (this.warning.toLowerCase() == 'warning') {
//			this.warning = 'ok';
//			alert('warning');
//			this.translateEditor.callRaiseWarning('warning');
//		}
		return true;		
	},
