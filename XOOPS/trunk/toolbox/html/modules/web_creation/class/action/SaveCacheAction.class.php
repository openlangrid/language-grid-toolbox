...
class SaveCacheAction extends AjaxAction {
	...
	public function execute() {
		...
		try {
			...
			$key = $service->registerContents($contents);
			
			$url = XOOPS_URL.'/modules/'.APP_DIR_NAME.'/?ml_lang=raw&action=displayCache&key='.$key;
			
			$this->buildSuccessResult($url);
		} catch (Exception $e) {
