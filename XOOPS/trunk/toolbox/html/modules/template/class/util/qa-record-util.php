...
class QaRecordUtil {
	...
	public static function buildRecord($record) {
		debugLog(print_r($record, true));
		if (isset($record->resourceName)) {
		return array(
			'questionId' => $record->id,
			'parameterIds' => $record->wordSetIds,
			'categoryIds' => $record->categoryIds,
				'matchValue' => $record->matchValue,
				'expressions' => self::toolboxVos2expressions($record->expressions, $record->resourceName)
		);
			...
			return array(
				...
				'parameterIds' => $record->wordSetIds,
				'categoryIds' => $record->categoryIds,
				'expressions' => self::toolboxVos2expressionsClassic($record->expressions)
			);
		}
	}
	 ...
	 */
	public static function toolboxVos2expressionsClassic($vos) {
		$expressions = array();
		foreach ($vos as $vo) {
	 ...
	 * @return array<String, String> $expressions
	 */
	public static function toolboxVos2expressions($vos, $resourceName = null) {
		$expressions = array();
		foreach ($vos as $vo) {
			$expressions[$vo->language] = $vo->expression;
			$expressions['resourceName'] = $resourceName;
		}
