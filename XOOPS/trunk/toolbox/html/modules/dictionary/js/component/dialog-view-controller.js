var DialogViewController = {
	...
	_getTypeId: function(dict) {
		...
		if(Const.Mode == "ParaphraseDictionary") return 5;		
		if(Const.Mode == "NormalizeDictionary") return 6;
		return 0;
	},
...
DialogViewController.CreateDictionary = {
	...
	create: function() {
		...
		if (!form["supportedLanguages[]"] || !Object.isArray(form["supportedLanguages[]"]) || form["supportedLanguages[]"].length < 2) {
			alert(Const.Message.Error.selectAtLeastTwoLanguages);
			return;
		} else if (Const.Mode == 'NormalizeDictionary' && form["supportedLanguages[]"].length >= 3) {
			alert(Const.Message.Error.tooMuchSelectedLanguages);
			return;
		}
	...
	toggleNormalizedLangs: function(checkbox) {
		if (Const.Mode != 'NormalizeDictionary') return;

		Form.getInputs(DialogViewController.$.down("form"),'checkbox').each(function(e){
			if (e.value == checkbox.value+'-normalized') {
