...
Object.extend(WordsViewController.Table, {
	...
	appendRowsAll: function(dictionaryContents) {
		...
		if (Const.Mode == 'NormalizeDictionary') {
			var normalizedColumn = langs.detect(function(e) { return e.endsWith('-normalized') });
			var lastNormalizedValue = '';
			dictionaryContents.each(function(record){
				if (lastNormalizedValue == record[normalizedColumn]) {
					WordsViewController.Table.insertItemForMultiValue(this.$.select("tbody tr").last(), langs, record);
				} else {
					WordsViewController.Table.appendRow(langs, record);
				}
				lastNormalizedValue = record[normalizedColumn];
			}.bind(this));
		} else {
			dictionaryContents.each(function(record){
				WordsViewController.Table.appendRow(langs, record);
	...
	drawHeader: function(languages) {
		...
		var template = (Const.Mode == 'NormalizeDictionary') ? TMPL.getHeaderCellFixForTableWords() : TMPL.getHeaderCellForTableWords();
		var header = languages.map(function(lang){
			...
			return template.sub(/<option[^>]*><\/option>/i, options).interpolate({
				...
				className: lang.endsWith('-normalized') ? 'normalized' : ''	
			});
	...
	appendRow: function(languages, contents){
		var row = languages.map(function(lang){
			var tmpl = (Const.Mode == 'NormalizeDictionary') ? TMPL.getCellForTableWordsMulti() : TMPL.getCellForTableWords();
			return tmpl.interpolate({
				langTag: lang,
				"word value": contents[lang].escapeHTML().gsub('"', "&quot;"),
				wordDisp: contents[lang].escapeHTML() || "--", 
				rowno: contents["row"],
				className: lang.endsWith('-normalized') ? 'normalized' : 'not'
			});
		}).join("") + TMPL.getRemoveCellForTableWords();
		this.$.down("tbody").insert("<tr class='row"+ contents["row"] +"'>" + row + "</tr>");
	},

	insertItemForMultiValue: function(row, languages, contents) {
		languages.each(function(lang){
			var li = TMPL.getListItemForTableWordsMulti().interpolate({
				langTag: lang,
				"word value": contents[lang].escapeHTML().gsub('"', "&quot;"),
				wordDisp: contents[lang].escapeHTML() || "--", 
				rowno: contents["row"]
			});
			row.down('td.' + lang + ' ul').insert(li);
		});
	},
	...
	add: function() {
		...
		var row = $R(0, header.cells.length-2).map(function(i){
			var lang = $(header.cells[i]).down("input").getValue();
			var tmpl = (Const.Mode == 'NormalizeDictionary') ? TMPL.getNewCellForTableWordsMulti() : TMPL.getNewCellForTableWords();
			return tmpl.interpolate({
				langTag: lang,
				tmpKey : counter,
				className: lang.endsWith('-normalized') ? 'normalized' : 'not'
	...
	addMultiValue: function(targetButton) {
		var counter = this.counter = (this.counter || 0) + 1;
		var header = this.$.down("thead tr");
		var targetRow = targetButton.up('tr');
		$R(0, header.cells.length-2).each(function(i){
			var lang = $(header.cells[i]).down("input").getValue();
			var ul = targetRow.down('.' + lang + ' ul');
			var value = lang.endsWith('-normalized') ? ul.select('li input').first().getValue() : "";

			var li = TMPL.getNewListItemForTableWordsMulti().interpolate({
				langTag: lang,
				tmpKey : counter,
				"word value": value.escapeHTML().gsub('"', "&quot;")
			});
			targetRow.down('.' + lang + ' ul').insert(li);
		});
	},
	...
	afterEditCellAndCopyToOther: function(targetInput, event) {
		this.afterEditCell(targetInput, event);
		if (targetInput.up('td').classNames().detect(function(e){ return e.endsWith('-normalized')})) {
			var value = targetInput.getValue();
			targetInput.up('li').siblings().each(function(e){
				e.down('input').setValue(value);
			});
		};
	},
