...
Language = {
	...
	colum2span: ['nb', 'be', 'zh-CN', 'zh-TW', 'en-GB', 'en-US', 'ga', 'nn', 'pt-BR', 'pt-PT', 'rm', 'gd'],
	...
	getNameByTag: function(tag){
		var hash = this.getLanguageMappings();
		return hash[tag] ? hash[tag] : tag; 
	},
	...
	getTagByName: function(name){
		var hash = this.getLanguageMappings();
		var langPair = $H(hash).detect(function(pair){ return pair.value == name; });
		return langPair ? langPair[0] : name;
	},
	...
	getLanguageMappings: function() {
		return Form.serialize($("supportedLanguages"),{hash: true});
	},
	
	isSupportedLanguage: function(langTag) {
		var hash = this.getLanguageMappings();
		if(!Object.isArray(langTag)) langTag = [langTag];
		return langTag.all(function(lang){
				return !!hash[lang];
		});	
	}
};
