...
class User_UserViewAction extends User_AbstractViewAction
{
	...
	function execute(&$controller, &$xoopsUser)
	{
		...
		$handler =& xoops_getmodulehandler('users');
		$ret = $handler->insert($this->mObject);

		if($ret===true){
			XCube_DelegateUtils::call('Legacy_Profile.SaveProfile', new XCube_Ref($ret), $this->mActionForm);
		}

        return $ret ? USER_FRAME_VIEW_SUCCESS : USER_FRAME_VIEW_ERROR;
	}
...
//HACK by domifara
		$profile_data = false;
		XCube_DelegateUtils::call('Legacy_Profile.GetProfile', new XCube_Ref($profile_data), $xoopsUser->get('uid'));
