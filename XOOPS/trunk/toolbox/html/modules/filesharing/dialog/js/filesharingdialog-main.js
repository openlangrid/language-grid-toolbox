...
//  ------------------------------------------------------------------------ //
/* $Id: filesharingdialog-main.js 4708 2010-11-09 08:38:56Z kitajima $ */

var FileSharingDialog = Class.create();
Object.extend(FileSharingDialog.prototype, Panel.prototype);
Object.extend(FileSharingDialog.prototype, {

	defaultOptions: {
		Id: '',
		dialogMode: 'Open',									// Open 参照 | Save 保存
		dialogBuilderClass: FileSharingDialogBuilder,		// ダイアログ組み立てクラス
		onOk: function(){},
		willHide: function(){},
	...
	initialize: function(id, options) {
		...
		FileSharingDialogFrame.clear();

		// 実行optionsを設定
		Object.extend(this.options, this.defaultOptions);

		// テキストリソースを上書き
		if (FileSharingDialog_Global.Text) {
			Object.extend(this.options.Text, FileSharingDialog_Global.Text);
		}

		// ダイアログモードによるビルダのスイッチ
		if (options.dialogMode == 'Save') {
			options.dialogBuilderClass = FileSharingSaveDialogBuilder;
		}
		...
		FileSharingDialogFrame.setOkEvent(this.options.onOk);
		FileSharingDialogFrame.setHideEvent(this.options.willHide);
		FileSharingDialogFrame.setFileSelectEvent(this.options.onFileSelect);
	},
