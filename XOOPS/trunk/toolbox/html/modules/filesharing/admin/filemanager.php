...
if( ! empty( $_POST['action'] ) && $_POST['action'] == 'delete' && isset( $_POST['ids'] ) && is_array( $_POST['ids'] ) ) {
	...
	foreach( $_POST['ids'] as $lid ) {
		filesharing_delete_files( "lid=".intval( $lid ) ) ;
	}
...
// Query
$rs = $xoopsDB->query( "SELECT count(l.lid) FROM $table_files l LEFT JOIN $table_cat c ON l.cid=c.cid WHERE $whr" ) ;
list( $numrows ) = $xoopsDB->fetchRow( $rs ) ;
$prs = $xoopsDB->query( "SELECT l.lid, l.title, l.submitter, l.ext, l.res_x, l.res_y, l.status FROM $table_files l LEFT JOIN $table_cat c ON l.cid=c.cid WHERE $whr ORDER BY l.lid DESC LIMIT $pos,$num" ) ;
...
$numbers = explode( '|' , $filesharing_perpage ) ;
$num_options = '' ;
...
filesharing_get_cat_options() ;

// Options for Selecting a category
$cat_options = filesharing_get_cat_options( 'title' , $cid , '--' , '----' ) ;
$cat_options_for_update = filesharing_get_cat_options( 'title' , 0 , '--' , _AM_OPT_NOCHANGE ) ;
...
while( list( $lid , $title , $submitter , $ext , $w , $h , $status ) = $xoopsDB->fetchRow( $prs ) ) {
	...
	if( in_array( strtolower( $ext ) , $filesharing_normal_exts ) ) {
		...
		$widthheight = $w > $h ? "width='$filesharing_thumbsize'" : "height='$filesharing_thumbsize'" ;
	} else {
	...
	echo "
		<td align='center' style='background-color:$bgcolor; margin: 0 px; padding: 3 px; border-width:0px 2px 2px 0px; border-style: solid; border-color:black;'>
			<table border='0' cellpadding='0' cellmargin='0'>
				<tr>
					<td></td>
					<td><img src='../images/pixel_trans.gif' width='$filesharing_thumbsize' height='1' alt='' /></td>
					<td></td>
				</tr>
				<tr>
					<td><img src='../images/pixel_trans.gif' width='1' height='$filesharing_thumbsize' alt='' /></td>
