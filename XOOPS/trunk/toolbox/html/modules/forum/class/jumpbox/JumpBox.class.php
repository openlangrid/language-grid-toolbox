...
//    	);
		...
		foreach ($categoryList as $category) {
			$result['categoryList'][] = array('id' => $category->id, 'text' => $this->_getText($category, $language));
			$forumRes = $this->mClient->getAllForums($category->id);
			$forumList = $forumRes['contents'];
			foreach ($forumList as $forum) {
				$result['forumList'][] = array('id' => $forum->id, 'parentId' => $forum->categoryId, 'text' => $this->_getText($forum, $language));
				$topicRes = $this->mClient->getAllTopics($forum->id);
				$topicList = $topicRes['contents'];
				foreach ($topicList as $topic) {
					$result['topicList'][] = array('id' => $topic->id, 'parentId' => $topic->forumId, 'text' => $this->_getText($topic, $language));
				}
			}
		}
