...
class QA_CategoriesObject extends XoopsSimpleObject {
	...
	function _loadExpressions() {
		...
		$mCriteria->add(new Criteria('qa_category_id', $this->get('id')));
	...
	private function _getExpressionsHandler() {
		require_once(dirname(__FILE__).'/QA_CategoryExpressionsHandler.class.php');
		$handler = new QA_CategoryExpressionsHandler($GLOBALS['xoopsDB']);
		return $handler;
	}
...
class QA_CategoriesHandler extends XoopsObjectGenericHandler {

	var $mTable = "qa_categories";
	var $mPrimary = "id";
	var $mClass = "QA_CategoriesObject";
