...
require_once dirname(__FILE__).'/../workflow/MultiSentenceBackTranslationCombinedWithBilingualDictionaryWithLongestMatchSearch.class.php';

class BestChoiceBackTranslationCombinedWithBilingualDictionaryWithLongestMatchSearch
	extends MultiSentenceBackTranslationCombinedWithBilingualDictionaryWithLongestMatchSearch
{
	...
	protected function _makeBindingAtomicEbmt() {
		...
		$this->_client->setBindingTree(sprintf($binding, $translator, $translator));
	}
	...
	protected function _makeBindingBestChoiceNoDictionary() {
		...
		for ($i = 0; $i < count($translators); $i++) {
			...
			$bindNodes[] = sprintf($bindTemp, '', 'BackwardTranslationPL'.($i+1), $translator);
		}
	...
	protected function _makeBindingBestChoice() {
		...
		for ($i = 0; $i < count($translators); $i++) {
			...
			$bindNodes[] = sprintf($bindTemp, $childlen, 'BackwardTranslationWithTemporalDictionaryPL'.($i+1), $this->getGridId() . ':TranslationCombinedWithBilingualDictionaryWithLongestMatchSearch');
	...
	protected function _makeBindingBestChoiceWithEbmt() {
		...
		for ($i = 0; $i < count($translators); $i++) {
			if (!$this->isEbmt($translators[$i])) {
				...
				$bindNodes[] = sprintf($bindTemp, $childlen, 'BackwardTranslationWithTemporalDictionaryPL'.($tc), $this->getGridId() . ':TranslationCombinedWithBilingualDictionaryWithLongestMatchSearch');
				...
				$bindNodes[] = sprintf($bindTemp, '', 'BackwardTranslationPL'.($pc), $serviceId);
