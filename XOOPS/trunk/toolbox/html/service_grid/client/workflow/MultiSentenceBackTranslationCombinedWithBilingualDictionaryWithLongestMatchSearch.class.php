...
require_once(dirname(__FILE__).'/../back_translation_with_temporal_dictionary/BackTranslationCombinedWithBilingualDictionaryWithLongestMatchSearch.class.php');
...
class MultiSentenceBackTranslationCombinedWithBilingualDictionaryWithLongestMatchSearch
	...
	function __construct() {
		debugLog('MultiSentenceBackTranslationCombinedWithBilingualDictionaryWithLongestMatchSearch construct');
		parent::__construct();
	}
	...
	public function _translate() {
		debugLog('MultiSentenceBackTranslationCombinedWithBilingualDictionaryWithLongestMatchSearch invoke.');
		$lastResponse = null;
		$resultArray = array();
		...
		debugLog($this->context->getSourceTextJoinStrategyType());
		...
		switch ($this->context->getSourceTextJoinStrategyType()) {
			...
			default:
				debugLog(print_r($sourceArray, 1));
				foreach ($sourceArray as $source) {
					$this->context->setSource($source);
					debugLog('%%%%%%%%%%%%%%%%%   '.$source.'   %%%%%%%%%%%%%%%%%%');
					if (empty($source)) {
						debugLog('source empty continue.');
						continue;
					}
					$lastResponse = parent::_translate();
					debugLog(print_r($lastResponse, 1));
					if ($lastResponse != null && $lastResponse['status'] != 'OK') {
						return $lastResponse;
					}
...
//					debugLog('#@#@#@ '.print_r($licenseInformationArray, true));
