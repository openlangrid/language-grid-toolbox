...
$xoopsOption['template_main'] = "apfilesharing_topten.html" ;
...
$crs = $xoopsDB->query( "SELECT cid,title FROM $table_folder WHERE pid=0 ORDER BY title" ) ;
$rankings = array() ;
$i = 0;
while( list( $cid , $cat_title ) = $xoopsDB->fetchRow( $crs ) ) {
	...
	while( list ( $lid , $cid , $title , $submitter , $hits , $rating , $votes ) = $xoopsDB->fetchRow( $prs ) ) {
		...
		$rankings[$i]['file'][] = array( 'lid' => $lid , 'cid' => $cid , 'rank' => $rank , 'title' => $title , 'submitter' => $submitter , 'submitter_name' => apfilesharing_get_name_from_uid( $submitter ) , 'category' => $catpath , 'hits' => $hits , 'rating' => number_format( $rating , 2) , 'votes' => $votes ) ;
		$rank ++ ;
	}
