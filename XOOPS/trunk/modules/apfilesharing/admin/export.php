...
// To imagemanager
	...
	$dst_table_folder = $xoopsDB->prefix( "imagecategory" ) ;

	// get src information
	$src_cid = intval( $_POST['cid'] ) ;
	$src_table_files = $table_files ;
	$src_table_folder = $table_folder ;

	// get storetype of the imgcat
	$crs = $xoopsDB->query( "SELECT imgcat_storetype,imgcat_maxsize FROM $dst_table_folder WHERE imgcat_id='$dst_cid'" ) or die( 'Invalid imgcat_id.' ) ;
...
if( $sysperm_handler->checkRight('system_admin', XOOPS_SYSTEM_IMAGE, $xoopsUser->getGroups() ) ) {
	...
	$apfilesharing_cat_options = apfilesharing_get_cat_options( 'title' , 0 , '--' , '----' ) ;
