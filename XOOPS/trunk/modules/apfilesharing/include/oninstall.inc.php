...
$mydirname = @$_SESSION['apfilesharing_mydirname'] ;
if( empty( $mydirname ) ) $mydirname = basename(dirname(dirname(__FILE__)));
// TODO
include_once XOOPS_ROOT_PATH."/modules/$mydirname/language/english/apfilesharing_constants.php" ;

if(!is_dir(XOOPS_ROOT_PATH."/uploaded_files_ap")){
	@mkdir(XOOPS_ROOT_PATH."/uploaded_files_ap");
}
if(!is_dir(XOOPS_ROOT_PATH."/uploaded_files_ap/data")){
	@mkdir(XOOPS_ROOT_PATH."/uploaded_files_ap/data");
}
...
//		GPERM_DELETABLE => _MD_ALBM_GPERM_G_DELETABLE ,
	...
	foreach( $global_perms_array as $perms_id => $perms_name ) {
		...
		$gperm->setVar("gperm_name", "apfilesharing_global");
