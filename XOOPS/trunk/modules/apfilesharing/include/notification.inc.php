...
function apfilesharing_notify_iteminfo($not_category, $item_id)
{
	...
	if ($not_category=="global") {
		...
		$sql = "SELECT title FROM ".$xoopsDB->prefix("apfilesharing_cat")." WHERE cid=\'$item_id\'";
		$rs = $xoopsDB->query( $sql ) ;
		list( $title ) = $xoopsDB->fetchRow( $rs ) ;
		$item["name"] = $title ;
		$item["url"] = "$mod_url/?page=viewcat&cid=$item_id" ;
	} else if( $not_category == "file" ) {
		// Assume we have a valid event_id
		$sql = "SELECT title FROM ".$xoopsDB->prefix("apfilesharing_files")." WHERE lid=\'$item_id\'";
		$rs = $xoopsDB->query( $sql ) ;
		list( $title ) = $xoopsDB->fetchRow( $rs ) ;
		$item["name"] = $title ;
		$item["url"] = "$mod_url/?page=file&lid=$item_id" ;
	}
