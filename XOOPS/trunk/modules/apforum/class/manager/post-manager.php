...
//		echo $sql;
	...
	public function getPosts($topicId, $offset, $limit) {
		...
		if(!$this->isadmin){
			$sql .= ' INNER JOIN (SELECT post_id FROM '.$postAuthTable.' WHERE groupid IN('.implode(',',$this->gid_arr).')) AS pa USING(post_id) ';
		}
...
/* 		if(!$this->isadmin){
			$sql .= ' INNER JOIN (SELECT DISTINCT post_id FROM '.$postAuthTable.' WHERE groupid IN('.implode(',',$this->gid_arr).')) AS pa USING(post_id) ';
		} */
		...
		$sql .= '   ON U.`uid` = P.`uid` ';
		$sql .= ' WHERE P.`post_id` IN ('.implode(',',$this->getAllowedPostIds()).') ';
		$sql .= ' AND P.`topic_id` = '.intval($topicId).' ';
...
//		return $this->posts;
	...
	public function getPost($postId) {
		...
		if ($row = $this->db->fetchArray($result)) {
			...
			$auth = $this->getPostAuth($row['post_id']);
...
/* 		$sql .= '	INNER JOIN ( ';
	...
	public function createPostAuth($postId,$authGroup){
		$postAuthTable = $this->db->prefix($this->moduleName.'_post_auth');
		foreach($authGroup as $groupId){
			...
			$sql = sprintf($sql, $postId, $groupId);
			$result = $this->db->query($sql);
			if (!$result) {
				die(_MD_D3FORUM_ERR_SQL.__LINE__);
			}
	...
	public function deletePostAuth($postId){
		$postAuthTable = $this->db->prefix($this->moduleName.'_post_auth');
		$sql  ='';
		$sql .=' DELETE FROM ';
		$sql .=' '.$postAuthTable.' ';
	...
	public function modifyPostAuth($postId,$authGroup){
		$this->deletePostAuth($postId);
		$this->createPostAuth($postId, $authGroup);
	}
	
	public function getPostAuth($postId){
		if($this->getAllUserPermission($postId)==1){
			$auth = $this->getGroupIds();
		}else{
			$postAuthTable = $this->db->prefix($this->moduleName.'_post_auth');
			$sql  = '';
			$sql .= ' SELECT groupid FROM '.$postAuthTable.' ';
			$sql .= ' WHERE post_id = '.intval($postId);
	...
	public function getParentId($id){
		$table = $this->db->prefix($this->moduleName.'_posts');
		$sql  = '';
		$sql .= ' SELECT topic_id FROM '.$table.' ';
		$sql .= ' WHERE post_id = '.intval($id);					
		$result = $this->db->query($sql);
	...
	public function getReplyId($id){
		$table = $this->db->prefix($this->moduleName.'_posts');
		$sql  = '';
		$sql .= ' SELECT reply_post_id FROM '.$table.' ';
		$sql .= ' WHERE post_id = '.intval($id);					
		$result = $this->db->query($sql);
		
		$replyId = 0;
		if($row = $this->db->fetchArray($result)){
			$replyId =  $row['reply_post_id'];
		}
		
		return $replyId;
	}
	
	public function CreateAllUserPermission($id,$all=1){
		$postAccessTable = $this->db->prefix($this->moduleName.'_post_access');
		$sql  = '';
		$sql .= ' INSERT INTO '.$postAccessTable.' VALUES (%d,%d) ';
		
		$sql = sprintf($sql, $id, $all);
	...
	public function SetAllUserPermission($id,$all=1){
		$postAccessTable = $this->db->prefix($this->moduleName.'_post_access');
		$sql ='';
		$sql.=' UPDATE '.$postAccessTable.' SET `all` = %d WHERE post_id = %d';
	...
	public function getAllUserPermission($id){
		$postAccessTable = $this->db->prefix($this->moduleName.'_post_access');
		$sql  ='';
		$sql .=' SELECT `all` FROM '.$postAccessTable;
		$sql .=' WHERE post_id = %d';
		$sql = sprintf($sql,$id);
		$result = $this->db->query($sql);
