 ...
 * @author kitajima
 */
var QaEditMasterWordPopupPanel = Class.create();
Object.extend(QaEditMasterWordPopupPanel.prototype, LightPopupPanel.prototype);
Object.extend(QaEditMasterWordPopupPanel.prototype, {
	...
	initEventListeners : function() {
		...
		if (this.resource) {
			(this.getWordIds()).each(function(wordId, i){
				this.addEvent('deleteClicked' + i, this.getDeleteId(wordId), 'click', this.deleteClicked.bindAsEventListener(this));
				this.addEvent('saveClicked' + i, this.getSaveId(wordId), 'click', this.saveClicked.bindAsEventListener(this));
				this.addEvent('cancelClicked' + i, this.getCancelId(wordId), 'click', this.cancelClicked.bindAsEventListener(this));
			}.bind(this));
		}
	},
	...
	getWordIds : function() {
		if (!this.wordIds) {
			...
			this.wordIds = Object.keys(words);
	...
	createCategoryRow : function() {
		var html = [];
		this.getWordIds().each(function(wordId){
			
			var attribute = (wordId == 0) ? 'style="display: none;"' : '';
			
			var source = Global.Words.getName(wordId, this.sourceLanguage);
			var target = Global.Words.getName(wordId, this.targetLanguage);
			
			if (this.editingCategoryId == wordId && this.changeFlag) {
			...
			html.push(new Template(this.Templates.categoryRow).evaluate({
				rowId : this.getRowId(wordId),
				attribute : attribute,
				unsavedId : this.getUnsavedId(wordId),
				sourceClass : this.SOURCE_CLASS,
				source : source,
				targetClass : this.TARGET_CLASS,
				target : target,
				deleteLabel : Global.Text.DELETE,
				deleteId : this.getDeleteId(wordId),
				editId : this.getEditId(wordId),
				editLabel : Global.Text.EDIT,
				saveId : this.getSaveId(wordId),
				save : Global.Text.SAVE,
				cancelId : this.getCancelId(wordId),
				cancel : Global.Text.CANCEL,
				saveCancelRowId : this.getSaveCancelRowId(wordId)
			}));
		}.bind(this));
		return html.join('');
	},
...
//			qa : this.resource.name,
			editMasterCategory : Global.Text.EDIT_BOUND_WORDS,
			xId : this.X_ID,
			addCategoryId : this.ADD_CATEGORY_ID,
			addCategory : Global.Text.ADD_BOUND_WORD,
	...
	onShowPanel : function() {
		this.stopEventObserving();
		new Ajax.Request(Global.Url.LOAD_WORDS, {
			...
			onSuccess : function(transport) {
				...
				for (var id in response.contents.categories) {
					var category = Global.Words.get(id);
					if (!category) {
						continue;
					}
					category.count = response.contents.boundWordSets[id].count;
					Global.Words.set(id, category);
				}
			}.bind(this)
		});
		this.initEventListeners();
	...
	contentsChangedProcess : function(){
		...
		if (!this.changeFlag) {
			this.cache = {};
			this.resource.languages.each(function(language){
				this.cache[language] = Global.Words.get(this.editingCategoryId)[language];
			}.bind(this));
			this.showSaveCancelRow(this.editingCategoryId);
			$(this.getUnsavedId(this.editingCategoryId)).show();
...
Object.extend(QaEditMasterWordPopupPanel.prototype, {
	...
	commitChanges : function() {
		...
		if (Object.isArray(words)) {
			Global.WordSets.get(this.wordSetId).words = {};
		}
		
		Global.WordSets.get(this.wordSetId).words[this.editingCategoryId] = category;
		Global.Words.set(this.editingCategoryId, category);

		if (this.newFlag) {
			this.wordIds.unshift(this.editingCategoryId);
			this.hasNew = false;
		}
	...
	contentsClicked : function(event) {
		...
		if (this.changeFlag && (this.editingCategoryId != wordId)) {
			...
			if (this.newFlag) {
				...
				$$('#' + this.ROW + this.editingCategoryId + ' td')[1].update(Global.Words.get(this.editingCategoryId)[this.sourceLanguage]);
				$$('#' + this.ROW + this.editingCategoryId + ' td')[2].update(Global.Words.get(this.editingCategoryId)[this.targetLanguage]);
			}
			this.discardChanges();
		}
	...
	deleteClicked : function(event) {
		...
		var parameter = Object.toQueryString({
			boundWordSetId : wordId,
			name : this.resource.name
		});
		new Ajax.Request(Global.Url.DELETE_MASTER_WORD, {
			postBody : parameter,
			onSuccess : function() {
...
QaEditMasterWordPopupPanel.prototype.Templates = {
