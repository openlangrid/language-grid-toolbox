...
class TopicManager extends AbstractManager {
	...
	public function getTopicsByForumId($forumId) {
		if (!$this->topics) {
			...
			$templateTypeTable = $this->db->prefix($this->moduleName.'_topics_template_type');
			...
			switch($sortkey){
				...
				case  7:
				case  8:$OrderSQL.='T6.template_type';break;
			...
			$sql .= '  T6.template_type AS topic_template_type, ';
			...
			$sql .= ' LEFT JOIN ( ';
			$sql .= '   SELECT ';
			$sql .= '   topic_id as tepmlate_topic_id, ';
			$sql .= '   template_type';
			$sql .= '   FROM ' . $templateTypeTable . ' ';
			$sql .= '   WHERE `language_code` = \''.$this->languageManager->getSelectedLanguage().'\' ';
			$sql .= ' ) AS T6 ON T6.tepmlate_topic_id = T2.topic_id ';
