...
if( ! defined( 'FSHARE_BLOCK_TOPNEWS_INCLUDED' ) ) {

define( 'FSHARE_BLOCK_TOPNEWS_INCLUDED' , 1 ) ;

function b_filesharing_topnews_show( $options )
{
	global $xoopsDB ;
	...
	$result = $xoopsDB->query( "SELECT l.lid , l.cid , l.title , l.ext , l.res_x , l.res_y , l.submitter , l.status , l.date AS unixtime , l.hits , l.rating , l.votes , l.comments , c.title AS cat_title FROM $table_files l LEFT JOIN $table_cat c ON l.cid=c.cid WHERE l.status>0 AND $whr_cat ORDER BY unixtime DESC" , $files_num , 0 ) ;
	$count = 1 ;
	while( $file = $xoopsDB->fetchArray( $result ) ) {
		...
		if( in_array( strtolower( $file['ext'] ) , $filesharing_normal_exts ) ) {
			...
			if( $filesharing_makethumb ) {
				...
				if( $width <= $filesharing_thumbsize ) 
...
function b_filesharing_topnews_edit( $options )
{
	global $xoopsDB ;
