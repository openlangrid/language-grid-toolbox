...
if( ! empty( $_POST['submit'] ) ) {
	...
	while( list( $lid , $ext , $w , $h ) = $xoopsDB->fetchRow( $result ) ) {
		...
		if( ! is_readable( "$files_dir/$lid.$ext" ) ) {
			...
			if( $removerec ) {
				filesharing_delete_files( "lid='$lid'" ) ;
		...
		if( $resize && ( $true_w > $filesharing_width || $true_h > $filesharing_height ) ) {
			$tmp_path = "$files_dir/filesharing_tmp_file" ;
			@unlink( $tmp_path ) ;
			rename( "$files_dir/$lid.$ext" , $tmp_path ) ;
			filesharing_modify_file( $tmp_path , "$files_dir/$lid.$ext" ) ;
			@unlink( $tmp_path ) ;
		...
		if( is_readable( "$thumbs_dir/$lid.$ext" ) ) {
			...
			if( $forceredo ) {
				$retcode = filesharing_create_icon( "$files_dir/$lid.$ext" , $lid , $ext ) ;
			} else {
				$retcode = 3 ;
			}
		} else {
...
$resize_radio = new XoopsFormRadioYN( _AM_RADIO_RESIZE." ({$filesharing_width}x{$filesharing_height})" , 'resize' , $resize ) ;
