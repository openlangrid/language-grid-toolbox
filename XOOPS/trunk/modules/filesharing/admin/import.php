...
// From filesharing*
if( ! empty( $_POST['filesharing_import'] ) && ! empty( $_POST['cid'] ) ) {
	...
	$src_files_dir = XOOPS_ROOT_PATH . $src_configs['filesharing_filespath'] ;
	$src_thumbs_dir = XOOPS_ROOT_PATH . $src_configs['filesharing_thumbspath'] ;
	// src table names
	$src_table_files = $xoopsDB->prefix( "filesharing{$src_dirnumber}_files" ) ;
	$src_table_cat = $xoopsDB->prefix( "filesharing{$src_dirnumber}_cat" ) ;
	$src_table_text = $xoopsDB->prefix( "filesharing{$src_dirnumber}_text" ) ;
	$src_table_votedata = $xoopsDB->prefix( "filesharing{$src_dirnumber}_votedata" ) ;
	...
	while( list( $src_lid , $ext ) = $xoopsDB->fetchRow( $rs ) ) {
		...
		if( $move_mode ) {
			// moving comments
			$sql = "UPDATE ".$xoopsDB->prefix('xoopscomments')." SET com_modid='$filesharing_mid',com_itemid='$lid' WHERE com_modid='$src_mid' AND com_itemid='$src_lid'" ;
			$xoopsDB->query( $sql ) ;

			// delete source files
			list( $files_dir , $thumbs_dir , $filesharing_mid , $table_files , $table_text , $table_votedata ,  $saved_files_dir , $saved_thumbs_dir , $saved_filesharing_mid , $saved_table_files , $saved_table_text , $saved_table_votedata ) = array(  $src_files_dir , $src_thumbs_dir , $src_mid , $src_table_files , $src_table_text , $src_table_votedata , $files_dir , $thumbs_dir , $filesharing_mid , $table_files , $table_text , $table_votedata ) ;
			filesharing_delete_files( "lid='$src_lid'" ) ;
			list( $files_dir , $thumbs_dir , $filesharing_mid , $table_files , $table_text , $table_votedata ) = array( $saved_files_dir , $saved_thumbs_dir , $saved_filesharing_mid , $saved_table_files , $saved_table_text , $saved_table_votedata ) ;		}
...
// From imagemanager
	...
	while( list( $image_id,$image_name,$image_nicename,$image_created,$image_display ) = $xoopsDB->fetchRow( $rs ) ) {
		...
		if( $imgcat_storetype == 'db' ) {
			...
			fwrite( $fp , $body ) ;
			fclose( $fp ) ;
			filesharing_create_icon( $dst_file , $lid , $ext ) ;
		} else {
			@copy( $src_file , $dst_file ) ;
			filesharing_create_icon( $src_file , $lid , $ext ) ;
		}
...
// From filesharing*
$mrs = $xoopsDB->query( "SELECT dirname FROM ".$xoopsDB->prefix("modules")." WHERE dirname like 'filesharing%'" ) ;
// get all instances of TinyD using newblocks table
$mrs = $xoopsDB->query( "SELECT distinct dirname FROM ".$xoopsDB->prefix("newblocks")." WHERE func_file='filesharing_rfile.php'" ) ;
while( list( $src_dirname ) = $xoopsDB->fetchRow( $mrs ) ) {
	...
	$filesharing_cat_options = filesharing_get_cat_options( 'title' , 0 , '--' , '----' , $xoopsDB->prefix( "filesharing{$src_dirnumber}_cat" ) , $xoopsDB->prefix( "filesharing{$src_dirnumber}_files" ) ) ;
	...
	echo "
		...
		<input type='submit' name='filesharing_import' value='"._GO."' onclick='return confirm(\""._AM_MB_IMPORTCONFIRM."\");' />
		</form>\n" ;
