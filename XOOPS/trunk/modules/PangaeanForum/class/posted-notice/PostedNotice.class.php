...
//  ------------------------------------------------------------------------ //
/* $Id: PostedNotice.class.php 4540 2010-10-07 04:01:57Z uehara $ */
...
class PostedNotice extends Toolbox_AbstractManager {
    ...
    public function notifyPostedEachTime($postId) {
		...
		$topic = $bbsAdp->getTopic($messages[0]->topicId);
		$xTopicAuthor = $userAdp->getUser($topic->get('uid'));
		if ($xTopicAuthor->get('uname') !== $messages[0]->creator) {
			$topicAuthor = $this->mHandler->findByUserId($topic->get('uid'));
			if ($topicAuthor !== false && $topicAuthor->getVar('notice_type') === 3) {
				$users[] = $topicAuthor;
			}
		}
		if (!$users) {
    ...
    public function notifyPostedOnceDay() {
		...
		foreach ($messages as $message) {
			$topic = $bbsAdp->getTopic($message->topicId);
			$xTopicAuthor = $userAdp->getUser($topic->get('uid'));
			if ($xTopicAuthor->get('uname') === $message->creator) {
				continue;
			}
			$topicAuthor = $this->mHandler->findByUserId($topic->get('uid'));
			if ($topicAuthor !== false && $topicAuthor->getVar('notice_type') === 4) {
				if (!isset($messageTbl[$topicAuthor->get('user_id')])) {
					$messageTbl[$topicAuthor->get('user_id')] = array(
							'User' => array(
								'xoopsUser' => $xTopicAuthor,
								'noticeConfig' => $topicAuthor
							),
							'messages' => array()
						);
				}
				$messageTbl[$topicAuthor->get('user_id')]['messages'][] = $message;
			}
		}
		foreach ($messageTbl as $m) {
			$this->sendNotify(array($m['User']), $m['messages']);
		}
    }

    protected function sendNotify($sendToUsers, $messages) {
//    	debugLog("--sendNotify start--");
//	foreach ($messages as $m) {
//		debugLog("message: ".$m->id);
//	}
//	foreach ($sendToUsers as $u) {
//    		debugLog("send to: ".$u['xoopsUser']->get('uname'));
//	}
//	debugLog("--sendNotify end--");
//	return ;
...
//			return $arr;
			return $this->mangleMessages($arr);
		}
		return false;
	}
...
//			return $a['contents'];
			return $this->mangleMessages($a['contents']);
		}
		return false;
	}
	...
	public function getTopic($topicId) {
		return $this->mClient->getTopic($topicId);
	}
	public function mangleMessages($messages) {
		$msgs = array();
		foreach ($messages as $m) {
			$u = $this->
				mUser->getUserByUname($m->text->creator);
			if ($u) $m->text->creator = $u;
			$msgs[] = $m;
		}
		return $msgs;
	}
...
class UserAdapter {
	...
	public function getUserByUname($uname) {
		$userArr = $this->mUserHandler->getObjects(new Criteria('uname', $uname));
		if ($userArr != null && is_array($userArr) && count($userArr) > 0) {
			return $userArr[0];
		}
		return false;
