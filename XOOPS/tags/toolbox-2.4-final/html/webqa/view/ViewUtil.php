...
//  ------------------------------------------------------------------------ //
class ViewUtil extends AbstractView {

	public static function getCategories($lang) {
		$qaManager =& new QAManager();
		$categories = $qaManager->getCategory();
		if (!$categories) return array();

		$return = array();
		foreach ($categories as $c) {
			$l = $c['language']; // original language

			$word = isset($c['name'][$lang]) ? $c['name'][$lang] : '';
			if ($word == '') {
				$word = '---';
				if (isset($c['name'][$l]) && $c['name'][$l] != '') {
					$word .= ' ('.$c['name'][$l].')';
				}
			}
			$return[$c['id']] = $word;
