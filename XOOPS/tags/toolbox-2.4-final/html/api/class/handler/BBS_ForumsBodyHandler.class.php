...
class BBS_ForumBodyObject extends XoopsSimpleObject {

	function BBS_ForumBodyObject() {
...
class BBS_ForumsBodyHandler extends Toolbox_CompositeKeyGenericHandler {

	var $mTable = "";
	var $mModName = "";
	var $mPrimary = "forum_id";
	var $mClass = "BBS_ForumBodyObject";
	var $mPrimaryAry = array('forum_id', 'language_code');
	
	public function __construct(&$db,$moduleName) {
		parent::__construct($db);
		$this->mTable = $db->prefix($moduleName."_forums_body");
	...
	function &getObjects($criteria = null, $limit = null, $start = null, $id_as_key = false){
		$ret = array();
		$forumTable = $this->db->prefix($this->mModName.'_forums');
		
		$sql  = "SELECT * FROM `" . $this->mTable . '`';
		$sql .= " WHERE forum_id in (select forum_id from ".$forumTable." where delete_flag = 0) ";
