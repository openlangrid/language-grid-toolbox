...
//  ------------------------------------------------------------------------ //
/* $Id: helper.php 3662 2010-06-16 02:22:17Z yoshimura $ */
...
function make_delegator($module_name, $submodule_name, $action) {
	$serviceInfo = new ServiceInfo($module_name, $submodule_name, $action);
	return new Delegator($serviceInfo);
}
...
class Delegator {
	...
	private function _targetFile() {
		...
		$f = XOOPS_TRUST_PATH."/modules/{$module}/actions/{$submodule}${action}.php";
		if (file_exists($f)) {
			return $f;
		}
		$f = XOOPS_ROOT_PATH."/modules/{$module}/actions/{$submodule}${action}.php";
		if (file_exists($f)) {
			return $f;
		}
		throw new Exception("Action target file is not found. in uicustomize helper.php");
	}
