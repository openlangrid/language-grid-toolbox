...
//  ------------------------------------------------------------------------ //
/* $Id: translation-path-panel.js 6005 2011-09-05 07:02:45Z mtanaka $ */
...
TranslationPathPanel.prototype = {
	...
	initialize: function() {
		...
		this._temp_dicts = [[], [], []];
		this._dict_flags = [[], [], []];
		this._morphemes_from = ['', '', ''];
		this._morphemes_to = ['','',''];
		this._similarity = ['', '', ''];
		this._parallel = [[], [], []];
		this._translationTemplates = [[], [], []];
	},
	...
	makeNewTranslationPathPanel: function(rootPanel,addPanel,lgServiceArray,PathArray,defaultSetting) {
		...
		for (var i = 0; i <= 2; i++) {
			...
			this._global_dicts[i] = (defaultSetting.global instanceof Array) ? defaultSetting.global.deepClone() : [];
			this._local_dicts[i] = (defaultSetting.local instanceof Array) ? defaultSetting.local.deepClone() : [];
			this._temp_dicts[i] = (defaultSetting.temp instanceof Array) ? defaultSetting.temp.deepClone() : [];
		}
	...
	_initializeSettingData: function(setting) {
		...
		if (setting.source_lang) {
			if (setting.translator_service_3 && setting.translator_service_3.length > 0) {
			...
			} else if (setting.translator_service_2 && setting.translator_service_2.length > 0) {
				data.lang1 = setting.source_lang;
				data.lang2 = setting.inter_lang_1;
			...
			if (setting.service3 && setting.service3.length > 0) {
				...
				AnalyzeSvc[2] = this._getValidAnalyzer(setting.morph_analyzer3,setting.lang3);
				AnalyzeSvc[3] = this._getValidAnalyzer(setting.morph_analyzer4,setting.lang4);
			} else if (setting.service2 && setting.service2.length > 0) {
				data.lang1 = setting.lang1;
				data.lang2 = setting.lang2;
				data.lang3 = setting.lang3;
				data.service1 = setting.service1;
		...
		for (var i = 0; i <= 2; i++) {
			...
			data.parallel[i] = slist.get('parallel'+String(i+1));
			data.translationTemplates[i] = slist.get('translationTemplates'+String(i+1));
			
			data.similarity[i] = slist.get('similarity'+String(i+1));
		...
		for (var i = 0; i <= 2; i++) {
			this._global_dicts[i] = (!!data.global_dicts[i]) ? data.global_dicts[i].split(',') : [];
			this._local_dicts[i] = (!!data.local_dicts[i]) ? data.local_dicts[i].split(',') : [];
			this._temp_dicts[i] = (!!data.temp_dicts[i]) ? data.temp_dicts[i].split(',') : [];
			this._morphemes_from[i] = data.morph_from[i];
			this._morphemes_to[i] = data.morph_to[i];
			this._similarity[i] = data.similarity[i];
			this._parallel[i] = (!!data.parallel[i]) ? data.parallel[i].split(',') : [];
			this._translationTemplates[i] = (!!data.translationTemplates[i]) ? data.translationTemplates[i].split(',') : [];
	...
	_createViewPanel: function(data) {
		...
		var sourceLanguages = this._langridServiceInformations['_sourceLanguageArray'];
		...
		var service1 = this.getServiceNameFromArray(this._langridServiceInformations['_translationServicesArray'], data.service1);
		var service2 = this.getServiceNameFromArray(this._langridServiceInformations['_translationServicesArray'], data.service2);
		var service3 = this.getServiceNameFromArray(this._langridServiceInformations['_translationServicesArray'], data.service3);
	...
	_createEditPanel: function(data) {
		this._servicePanelElements = [];
		this._blockPanelArray = [];
	...
	_createFirstBlockPanel: function(data) {
		...
		if (data.service1.split) {
			data.service1 = data.service1.split(',');
		}
		
		data.service1.each(function(service) {
			servicePanel.setActiveServiceId(service);			
		});
		//*****Advanced options***********
		...
		if (data.service2 && data.service2.length > 0) {
			this._checkBoxElements.checkbox1.checked = true;
		}else{
	...
	_createSecondBlockPanel: function(data) {
		...
		if (data.service2.split) {
			data.service2 = data.service2.split(',');
		}
		data.service2.each(function(service) {
			servicePanel.setActiveServiceId(service);			
		});
		//*****Advanced options***********
		var dictdiv = document.createElement('div');
		...
		if (data.service3 && data.service3.length > 0) {
			this._checkBoxElements.checkbox2.checked = true;
		}else{
			this._checkBoxElements.checkbox2.checked = false;
		}
	...
	_createThirdBlockPanel: function(data) {
		...
		data.service3.each(function(service) {
			servicePanel.setActiveServiceId(service);			
		});
		//*****Advanced options***********
		var dictdiv = document.createElement('div');
...
//			}
	...
	getServiceNameFromArray: function(dataArray, dataKey){
		
		var dataKeys = (dataKey.split) ? dataKey.split(',') : dataKey;
		
		ret = [];
		dataArray.each(function(obj,i){
			if(dataKeys.indexOf(obj.service_id) != -1){
				ret.push(obj.service_name);
			}
		});
	...
	updateEditElement:function(){
		try{
			...
			if (!this._selfPanel || !$(this._selectBoxElements.lang3)) {
				return;
			}
			...
			if (!this._servicePanelElements[0].hasSetting()) {
				return;
			}
			...
			if (!this._servicePanelElements[1].hasSetting()) {
				return;
			}
			
			//--service2_on
			var minus1 = $(this.elementsIds.panel+":minus1");
			if (this._checkBoxElements.checkbox2.checked) {
				$(this.elementsIds.panel+":minus2").show();
				
				minus1.removeClassName("btn");
				minus1.addClassName("btn-disable");
				minus1.removeClassName("btn-mk");
				minus1.addClassName("btn-mk-disable");
				...
				if ($(this._selectBoxElements.lang3).value != "") {
					...
					minus1.removeClassName("btn-disable");
					minus1.addClassName("btn");
					minus1.removeClassName("btn-mk-disable");
					minus1.addClassName("btn-mk");
				}
	...
	_onSaveButtonClicked: function(ev) {
		if(!Element.hasClassName(Event.findElement(ev,'a'),'btn-disable')){
			...
			// EBMT & ParallelTextチェック
			for (var i = 0; i < 3; i++) {
				if (this.hasEBMT(i) && ((this._parallel[i].length + this._translationTemplates[i].length) == 0)) {
					validMessage = Const.Message.ParallelEBMTError;
					Element.scrollTo($(this._selfPanel));
					this.errorView(true);
					this.showMessageBox(validMessage);
					return;
...
//		new Ajax.Request('./ajax/save-user-setting.php', {
		new Ajax.Request(Resource.url.saveSetting, {
			method: 'post',
			parameters: hash,
			asynchronous:false,
	...
	updateViewPanel:function () {
		var data = this._initializeSettingData(this._buildData());
		this._preData = data;
		var viewPanel = this._createViewPanel(data);
	...
	reverseValue:function() {
		...
		if (this._preData.service3 && this._preData.service3.length > 0) {
			...
			data.similarity[0] = this._preData.similarity[2];
			data.similarity[1] = this._preData.similarity[1];
			data.similarity[2] = this._preData.similarity[0];
			
			data.parallel[0] = this._preData.parallel[2];
			data.parallel[1] = this._preData.parallel[1];
			data.parallel[2] = this._preData.parallel[0];
			
			data.translationTemplates[0] = this._preData.translationTemplates[2];
			data.translationTemplates[1] = this._preData.translationTemplates[1];
			data.translationTemplates[2] = this._preData.translationTemplates[0];
		} else if (this._preData.service2 && this._preData.service2.length > 0) {
			...
			data.similarity[0] = this._preData.similarity[1];
			data.similarity[1] = this._preData.similarity[0];

			data.parallel[0] = this._preData.parallel[1];
			data.parallel[1] = this._preData.parallel[0];

			data.translationTemplates[0] = this._preData.translationTemplates[1];
			data.translationTemplates[1] = this._preData.translationTemplates[0];
		} else {
			...
			data.similarity[0] = this._preData.similarity[0];
			data.parallel[0] = this._preData.parallel[0];
			data.translationTemplates[0] = this._preData.translationTemplates[0];
		}
	...
	_onDictionaryButtonClicked: function(ev) {
		...
		popupDictionaryPanel.setParams(
			...
			this._similarity[id],
			this._parallel[id],
			this._translationTemplates[id]
		);
		
		popupDictionaryPanel.both = this._isBoth();
	...
	_isBoth: function() {
		try {
			return $$('allow-cell')[0].value == '<>';
		} catch(e) {
			return ($(this.elementsIds.flow1).value == 'both');
		} finally {
			return false;
		}
	},
	...
	onDictionaryClickHandler: function(level, did, checked, srcType) {
		switch (srcType) {
			case "GLOBAL":
				if (checked) {
			...
			case "SIMILARITY":
				if (checked) {
					this._similarity[level] = did;
				}
				break;
				
			case "PARALLELTEXT":
				var index = this._parallel[level].indexOf(did);
				
				if (checked && index == -1) {
					this._parallel[level].push(did);
				} else if (!checked && index != -1) {
					this._parallel[level].splice(index, 1);
				}
				
				this._parallel[level].sort();
				break;
				
			case "TRANSLATION_TEMPLATE":
				var index = this._translationTemplates[level].indexOf(did);
				
				if (checked && index == -1) {
					this._translationTemplates[level].push(did);
				} else if (!checked && index != -1) {
					this._translationTemplates[level].splice(index, 1);
				}
				
				this._translationTemplates[level].sort();
				break;
		}
	},
	...
	makeServiceViewObj: function(services, dictbtn){
		var wrapper = document.createElement('div');
		
		for (var i = 0, length = services.length; i < length; i++) {
			var serviceOuter = document.createElement('div');
			serviceOuter.addClassName('service-cell');
			serviceOuter.innerHTML = services[i];
			
			if (i == (length-1)) {
				var dictionaryButton = document.createElement('span');
				dictionaryButton.setStyle({
					'float' : 'right'
				});
				dictionaryButton.appendChild(dictbtn);
				serviceOuter.appendChild(dictionaryButton);

				var br = document.createElement('br');
				br.setStyle({
					"clear" :"both"
				});
				serviceOuter.appendChild(br);
			}
			
			wrapper.appendChild(serviceOuter);
		}
	...
	getSetting: function() {
		...
		if (data) {
			...
			return $H(this._preData).toQueryString();
		}
	},
	...
	validate: function() {
		...
		if ((data.lang1 && data.lang2 && data.service1 && data.service1.length > 0) == false) {
			return Const.Message.SaveOnError1;
		}

		if (this._checkBoxElements.checkbox1.checked == true) {
			if ((data.lang2 && data.lang3 && data.service2 && data.service2.length > 0) == false) {
				return Const.Message.SaveOnError1;
			}
		}
		...
		} else if (data.service2 && data.service2.length > 0){
	...
	_buildData: function() {
		try {
			var data = {
				...
				service1: this._servicePanelElements[0].getSetting().join(','),
				service2: this._servicePanelElements[1].getSetting().join(','),
				service3: this._servicePanelElements[2].getSetting().join(','),
				...
				similarity1: this._similarity[0],
				similarity2: this._similarity[1],
				similarity3: this._similarity[2],
				parallel1: this._parallel[0].join(','),
				parallel2: this._parallel[1].join(','),
				parallel3: this._parallel[2].join(','),
				translationTemplates1: this._translationTemplates[0].join(','),
				translationTemplates2: this._translationTemplates[1].join(','),
				translationTemplates3: this._translationTemplates[2].join(','),
			...
			data.similarity1 = this._similarity[0];
			data.similarity2 = this._similarity[1];
			data.similarity3 = this._similarity[2];
			data.parallel1 = this._parallel[0].join(',');
			data.parallel2 = this._parallel[1].join(',');
			data.parallel3 = this._parallel[2].join(',');
			data.translationTemplates1 = this._translationTemplates[0].join(',');
			data.translationTemplates2 = this._translationTemplates[1].join(',');
			data.translationTemplates3 = this._translationTemplates[2].join(',');
	...
	_checkChange: function() {
		...
		for (var i = 1; i <= 4; i++) {
			if (data1['lang' + i] != $(this._selectBoxElements['lang' + i]).value) {
				return true;
			}
		}

		if (data1.flow != $(this._selectBoxElements.flow1).value) {
			return true;
		}
		
		for (var i = 0; i < 3; i++) {
			var a = data1['service' + (i+1)].clone().sort().join('');
			var b = this._servicePanelElements[i].getSetting().clone().sort().join('');
			if (a != b) {
				return true;
			}
		}
		...
		for (var i = 0; i <= 2; i++) {
			if (data1.dict_flags[i] != this._dict_flags[i]) {
				return true;
			}
			...
			if (data1.similarity[i] != this._similarity[i]) {
				return true;
			}

			if (data1.parallel[i].split(',').sort().join('') != this._parallel[i].clone().sort().join('')) {
				return true;
			}

			if (data1.translationTemplates[i].split(',').sort().join('') != this._translationTemplates[i].clone().sort().join('')) {
				return true;
			}
		}
	...
	_getValidAnalyzer: function (service_id,lang) {
		if (!this.isSupportedAnalyzer(service_id,lang)) {
			return getDefaultMorphologicalAnalyzer(lang);
		} else {
			return service_id;
		}
	},
	...
	__sort_array: function(ary, head, tail) {
		...
		if (head < i - 1) {
			this.__sort_array(ary, head, i - 1);
		}
		
		if (j + 1 < tail) {
			this.__sort_array(ary, j + 1, tail);
		}
	...
	// 0,1,2
	getTranslators: function(index) {
		var data = this._buildData();
		
		try {
			return data['service' + (index-0+1)].split(',');
		} catch (e) {
			return data['service' + (index-0+1)];
		}
	},
	
	hasEBMT: function(index) {
		var translators = this.getTranslators(index);
		
		if (!translators) {
			return false;
		}
		
		return !!translators.find(function(t) {
			return LangridServices.isEBMT(t);
		})
	},
