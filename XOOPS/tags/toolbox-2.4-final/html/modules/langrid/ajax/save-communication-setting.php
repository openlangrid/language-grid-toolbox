...
if ($mode != 'COMMUNICATION') {
	die('Modes are different for Communication.');
}
...
foreach ($postData as $data) {
	...
	$ids = explode(',', $data['id']);
	if (count($ids) == 1) {
		$afterIds = $pathSetting->saveCommunicationSetting($data);
		if ($data['flow1'] == 'both') {
			$data2 = postDateRevs($data);
			$data2['id'] = '';
			$tmp = $pathSetting->saveCommunicationSetting($data2);
			if ($tmp) {
		...
		$data1 = $data;
		$data1['id'] = $ids[0];
		$afterIds = $pathSetting->saveCommunicationSetting($data1);
		$data2 = postDateRevs($data);
		$data2['id'] = $ids[1];
		if ($data2['flow1'] == 'left') {
			$data2['isDelete'] = 'yes';
