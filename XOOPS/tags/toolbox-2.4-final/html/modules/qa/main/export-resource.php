...
foreach ($records as $record) {
	$answerIds = array();
	for ($j = $i, $length = $i + count($record->answers); $j < $length; $j++) {
		$answerIds[] = $j + 1;
	}
	$cells[$i] = array(
		'id' => $i,
		'type' => 'question',
		'answers' => implode(',', $answerIds),
		'categories' => implode(',', $record->categoryIds)
	);
	foreach ($record->question as $question) {
		$cells[$i][$question->language] = $question->expression;
	}
	foreach ($record->answers as $answerExps) {
		$i++;
		$cells[$i] = array(
			'id' => $i,
			'type' => 'answer',
...
$hrow = array('id', 'type', 'answers', 'categories');
...
foreach ($cells as $rowNumber => $row) {
	...
	$brow[] = $row['id'];
	$brow[] = $row['type'];
	$brow[] = isset($row['answers']) ? $row['answers'] : '';
