...
Object.extend(QaEditSelectCategoryPopupPanel.prototype, {
	...
	createCategoryCheckBoxHtml : function() {
		...
		if (!this.resource.categoryIds.length) {
			return '<span style="color:red;">' + Global.Text.NO_CATEGORY_DEFINED + '</span>';
		}
	...
	getBody : function() {
		return new Template(this.Templates.base).evaluate({
			editCategory : Global.Text.SELECT_CATEGORY,
...
QaEditSelectCategoryPopupPanel.prototype.Templates = {
	base : '<table id="qa-select-category-table">'
		...
		+ '<tr><td><b class="qa-common-popup-new-title">#{editCategory}</b><b style="display: none;"><span id="#{questionId}">#{question}</span></b></td></tr>'
