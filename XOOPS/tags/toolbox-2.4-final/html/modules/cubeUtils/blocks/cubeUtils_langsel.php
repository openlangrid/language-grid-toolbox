...
$html = '';
$html .= '
<select name="'.CUBE_UTILS_ML_PARAM_NAME.'" onChange="SelLang_jumpMenu(\'parent\',this,0)">
%s
</select>
';

$clang = preg_replace('/[^a-zA-Z-]+/u', '', (isset($_COOKIE['ml_lang']) ? $_COOKIE['ml_lang'] : 'en'));
...
//    if ($langs[$i] == _LANGCODE) $opts .= " selected";
    if ($langs[$i] == $clang) {$opts .= 'selected';}
    $opts .= '>'.$langnames[$i].'</option>';
}

$html = sprintf($html, $opts);
