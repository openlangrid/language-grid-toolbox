...
function filesharing_notify_iteminfo($not_category, $item_id)
{
	...
	if ($not_category=="global") {
		...
		$sql = "SELECT title FROM ".$xoopsDB->prefix("filesharing_cat")." WHERE cid=\'$item_id\'";
		...
		$sql = "SELECT title FROM ".$xoopsDB->prefix("filesharing_files")." WHERE lid=\'$item_id\'";
		$rs = $xoopsDB->query( $sql ) ;
		list( $title ) = $xoopsDB->fetchRow( $rs ) ;
		$item["name"] = $title ;
