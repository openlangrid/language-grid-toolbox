...
//  ------------------------------------------------------------------------ //
/* $Id: filesharingdialog-body-panel.js 4583 2010-10-13 09:53:46Z yoshimura $ */

var FileSharingDialogBodyPanel = Class.create();
Object.extend(FileSharingDialogBodyPanel.prototype, BodyPanel.prototype);
Object.extend(FileSharingDialogBodyPanel.prototype, {

	idPrefix: 'filesharingdialog-dialog-container-',
	...
	load: function() {
		new Ajax.Request(
			FileSharingDialog_Global.Url.FileList,
			{
	...
	reload: function(cid) {
		new Ajax.Request(
			new Template(FileSharingDialog_Global.Url.FolderList).evaluate({id:cid}),
			{
				onSuccess: function(xml) {
					this.onSuccess_draw(xml.responseText.evalJSON());
				}.bind(this)
...
//		FileSharingDialogFrame.setCurrentFolderId(jsonData.current);

		var navi = '';
		jsonData.parents.each(function(p, i){
			FileSharingDialogFrame.setCurrentFolder(p);

			var linkid = this._getParentFolderId(p.id);
			var folderLbl = p.name;
			if (p.id == 1) {
				folderLbl = FileSharingDialog_Global.Text.RootFolerLabel;
			}
			navi += new Template(this.Templates.naviItem).evaluate({
				name: folderLbl,
				id: linkid,
				href: link = new Template(FileSharingDialog_Global.Url.FolderList).evaluate({id: p.id})
			});
		...
		jsonData.list.each(function(row, i) {
			...
			if (row.type == 'folder') {
				...
				link = new Template(FileSharingDialog_Global.Url.FolderList).evaluate({id: row.id});
				...
				link = new Template(FileSharingDialog_Global.Url.FileDonwload).evaluate({id: row.id});
				linkid = '';
			}
			var canRead = (row.canRead ? FileSharingDialog_Global.Msg.HasPermissionIcon : '-');
			var canEdit = (row.canEdit ? FileSharingDialog_Global.Msg.HasPermissionIcon : '-');
...
//		this.Event.onLoaded.bind(this);
	...
	onFileListLoaded: function() {
		if (FileSharingDialogFrame.getCurrentFolder().canEdit) {
			...
			$(this._getUploadButtonId()+'_msg').innerHTML = FileSharingDialog_Global.Msg.PermissionErrorNoEdit;
		}
	}
});

Object.extend(FileSharingDialogBodyPanel.prototype.Templates, BodyPanel.prototype.Templates);
Object.extend(FileSharingDialogBodyPanel.prototype.Templates, {
...
FileSharingDialogBodyPanel.prototype.Event = {

	onLoaded: function() {
		FileSharingDialogFrame.notifyLoadedFileLists();
	},
	...
	onUploadClicked: function(event) {
		...
		if (FileSharingDialogFrame.getCurrentFolder().canEdit) {
			...
			var url = new Template(FileSharingDialog_Global.Url.FileUpload).evaluate({id: this.jsonData.current});
...
//		var fileid = $F(elem.id);
		FileSharingDialogFrame.notifyFileSelect(item);
	},
