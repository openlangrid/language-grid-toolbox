var WordTableController = {
	...
	onChangeDisplayLanguage: function(targetSelectElement) {
		...
		var oldLanguage = $(targetSelectElement).up("th").classNames().toString();
	...
	_swapLanguage: function(langTagA, langTagB) {
		...
		this.$.select("thead th").each(function(e){
			var lang = e.classNames().toString();
	...
	_getLanguageListCurrentOrder: function() {
		return this.$.select("thead th").map(function(th){
			return th.classNames().toString();
		}).without("remove", "resource-name");
	}
};
