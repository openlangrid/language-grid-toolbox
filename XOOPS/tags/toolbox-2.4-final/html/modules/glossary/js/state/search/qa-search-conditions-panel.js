...
Object.extend(QaSearchConditionsPanel.prototype, {
	...
	createLanguageSelector : function(elemId) {
		var html = [];
		var lang = this.getLanguages();
		LanguageUtils.sort(lang);

		if (lang.length == 0) {
			html.push('<select id="'+elemId+'" disabled>');
			html.push('</select>');
			html.push('<span style="color:red;">' + Global.Text.WARNING_NO_LANGUAGES_YOU_CAN_SELECT + '</span>');
		} else {
			html.push('<select id="'+elemId+'">');
	...
	updateLanguageSelectors : function() {
		$(this.KEYWORD_LANGUAGE_WRAPPER_ID).update(this.createLanguageSelector(this.KEYWORD_LANGUAGE_ID));
		$(this.ORDER_LANGUAGE_WRAPPER_ID).update(this.createLanguageSelector(this.ORDER_LANGUAGE_ID));
