...
//		echo $sql;
	...
	public function getPosts($topicId, $offset, $limit) {
		...
		$sql  = '';
		$sql .= ' SELECT post_id, file_id, file_name, file_size, uid, name, uname, language_code, description, poster_ip, post_original_language, post_time, post_order, delete_flag, update_date, update_time  FROM (('.$postsTable;
		...
		$sql .= ' LEFT JOIN (SELECT uid, name, uname FROM '.$userTable.') AS T2 USING (uid)) ';
