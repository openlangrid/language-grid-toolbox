...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	$params = $categoryManager->deleteCategory($categoryId);
	redirect_header(XOOPS_URL.'/modules/'.$mydirname.'/');
} else {
	$category = $categoryManager->getCategory($_GET['categoryId']);
	$xoopsTpl->assign( array(
		'category' => $category,
	));
	include XOOPS_ROOT_PATH.'/header.php';
	$xoopsOption['template_main'] = $mydirname.'_main_category_delete_form.html';
