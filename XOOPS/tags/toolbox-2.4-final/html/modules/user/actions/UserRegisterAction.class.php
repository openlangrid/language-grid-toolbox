...
class User_UserRegisterAction extends User_Action
{
	...
	{
		$action = $controller->mRoot->mContext->mRequest->getRequest('action');

		if ($action == 'back') {
			$this->mActionForm = unserialize($_SESSION['user_register_actionform']);
			if (is_object($this->mActionForm)) {
...
//		$renderSystem->setAttribute('displaylangs', $Languages);
		$Languages = array();
		$LangVal = explode(",",CUBE_UTILS_ML_LANGNAMES);
		$LangLabel = explode(",",CUBE_UTILS_ML_LANGDESCS);
		for($i=0; $i<count($LangVal); $i++) {
			$obj =& new DisplayLanguage();
			$obj->set('label', $LangLabel[$i]);
			$obj->set('value', $LangVal[$i]);
			$Languages[] = $obj;
		}
		$renderSystem->setAttribute('displaylangs', $Languages);
	}
}
class DisplayLanguage extends XoopsObject
{
	function DisplayLanguage()
	{
		$this->initVar('label', XOBJ_DTYPE_STRING, null, false);
        $this->initVar('value', XOBJ_DTYPE_STRING, null, false);
	}
}
