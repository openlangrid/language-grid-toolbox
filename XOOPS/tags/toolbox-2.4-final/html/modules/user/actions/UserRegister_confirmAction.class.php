...
class User_UserRegister_confirmAction extends User_Action
{
	...
	function executeViewInput(&$controller,&$xoopsUser,&$render)
	{
		...
		$timezone =& $handler->get($this->mRegistForm->get('timezone_offset'));
		$render->setAttribute('posted_timezone',$timezone->getVar('zone_name'));

		//$langNm = $GLOBALS['cubeUtilMlang']->getLangName($this->mRegistForm->get('display_language'));
		$lang = $this->mRegistForm->get('display_language');
		$LangVal = explode(",",CUBE_UTILS_ML_LANGNAMES);
		$LangLabel = explode(",",CUBE_UTILS_ML_LANGDESCS);
		$langs = array_flip($LangVal);

		$render->setAttribute('display_language_name', $LangLabel[$langs[$lang]]);
	}
