...
class User_LostPass1MailBuilder
{
	...
	{
		$this->mMailer->setSubject(_MD_USER_MAIL_LOST_PASSWORD_SUBJECT1);
	}
	...
	function setBody($user,$xoopsConfig,$extraVars)
	{
		...
		$this->mMailer->assign("NEWPWD_LINK", XOOPS_URL . "/lostpass.php?email=" . $email . "&code=" . substr($user->get("pass"), 0, 5));
	}
...
class User_LostPass2MailBuilder extends User_LostPass1MailBuilder
{
	...
	function setSubject($user, $xoopsConfig)
	{
		$this->mMailer->setSubject(_MD_USER_MAIL_LOST_PASSWORD_SUBJECT2);
	}
