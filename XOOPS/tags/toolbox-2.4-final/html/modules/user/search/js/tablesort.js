...
window.onload = function() {
	TableSort.init();
	//Event.observe($('load-dictionary-table-area'),"change",resetSort,false);
...
TableSort.init = function() {
	...
	arguments = Array("userList");

	// Initialise only the specified tables.
	var table;
	for (var x = 0; x < arguments.length; x++) {
		table = document.getElementById(arguments[x]+"-table");
		if (table) {
			//var tbody = document.getElementById(arguments[x]+"-body");
			//table.appendChild(tbody.cloneNode(true));
			TableSort.tables.push(arguments[x]);
...
TableSort.initTable = function(t) {
	var table_arg = TableSort.tables[t];
	var tHead = document.getElementById(table_arg+"-head");

	if (tHead) {
		for (var y = 0; y < tHead.rows.length; y++) {
			for (var x = 0; x < tHead.rows[y].cells.length; x++) {
				TableSort.linkCell(tHead.rows[y].cells[x], t, x, y);
			}
		}
	}
...
TableSort.click = function(table, column, mode) {
	...
	var table_arg = TableSort.tables[table];
	var tBody = document.getElementById(table_arg+"-body");
...
//	var className = 'color1';
	for (y = 0; y < cellDictionary.length; y++) {
		...
		var MyRow = cellDictionary[i][1];
		className = (y % 2 == 1) ? 'color2' : 'color1';

//		if (MyRow.cells.length > 0) {
			cells = MyRow.cells;
			MyRow.children[0].innerHTML=y+1;
//			for(c in MyRow.cells){
//				MyRow.cells[c].className = className;
//			}
//		} else { // Dodge Safari 1.0.3 bug
//			for(c in MyRow.childNodes){
//				MyRow.childNodes[c].setAttribute("className",className);
//				MyRow.childNodes[c].setAttribute("class",className);
//			}
//		}
