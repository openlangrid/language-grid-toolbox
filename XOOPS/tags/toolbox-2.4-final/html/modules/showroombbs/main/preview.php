...
if ( isset( $_POST['phaze'] ) && $_POST['phaze'] == 'post' ) {
	// necessary condition
	$necessaryCondition = array( 'category_create' => array( 'admin' => true 
			),
		'category_edit' => array( 'admin' => true 
			),
		'forum_create' => array( 'admin' => true 
			),
		'forum_edit' => array( 'admin' => true 
			),
		'topic_create' => array(
			),
	...
	switch ( $typeCode ) {
		...
		case 'topic_create':
			...
			$message = $bbsClient -> postMessage( $topic['contents'] -> id, $messageExpressions );
		...
		case 'post_create':
			...
			if(!is_array($resultSetFileRecord)){
				$message = $bbsClient -> postMessage( $id, $expressions ,null,null);
		...
		case 'post_reply':
			...
			if(!is_array($resultSetFileRecord)){
				...
				$message = $bbsClient -> postMessage( $id, $expressions ,null,$parent_id);
				require_once dirname( __FILE__ ) . '/../class/manager/post-manager.php';
				$postManager = new PostManager();
				$post = $postManager -> getPost( $message['contents'] -> id );
				$page = floor( ( $post -> getNumber()-1 ) / 20 ) + 1;
