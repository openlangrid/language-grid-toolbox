...
function initWorkspace(){
	...
	targetArea.editor.addListener("editorWindowBlur", windowBlur);
...
function translate(){
	voiceManager.init();
	if (publisher.isFreeTime && !sourceArea.isEmpty()) {
		doTranslation(true, false);
	}
}
function backTranslate(){
	if (publisher.isFreeTime && !targetArea.isEmpty()) {
...
function getTargetLangTag(){
	...
	if (targetPair.values().indexOf($('targetLang').value) == -1)
		voiceManager.init();
...
function clearText(ev) {
	if(publisher.isFreeTime){
		...
		clearInfoData();
		licenseArea.clear();
		voiceManager._viewController.hidePlayer();
	}
}
