...
var Publisher = Class.create( {
		...
		// warning
		callRaiseWarning :function(error) {
			if (unload) return;
			this.endProcess(error);
			alert(Const.Message.Error.Warning);
		},
		startTimer : function() {
			if(this.translator.isIntermediate()) {
		...
		endProcess : function(message) {
			...
			// Voice
			//voiceManager.setSource(this.ta.getEditorText());
			//voiceManager.setSources(this.ta.getTexts());
			voiceManager.setSource(this.ta.getVoiceText());
			voiceManager.setSources(this.ta.getVoiceTexts());
			voiceManager.create();
		},
