...
//  ------------------------------------------------------------------------ //
require_once(dirname(__FILE__).'/../class/TemplatedBBSClient.class.php');
...
function getTextByTag($source, $lang) {
    $matches;
    preg_match_all('/\{((?:[a-z]{2}(?:-[A-Z]{2})?)|\*)\}(.*?)\{\/\1\}/',
                   $source, $matches, PREG_SET_ORDER);
    foreach ($matches as $mth) {
        if ($mth[1] == $lang) {
            return $mth[2];
        } else if ($mth[1] == '*' || $mth[1] == 'en') {
            $default = $mth[2];
        }
    }
...
$bbsClient = new TemplatedBBSClient(USE_TABLE_PREFIX);
	...
	switch ( $typeCode ) {
		...
		case 'topic_create':
			...
			foreach ( $_POST['contents'] as $languageCode => $contents ) {
                ...
                if (@$contents["type"]) {
                    $templateTypeSet = $contents["type"];
                }
			}

            $templateTypes = array();
            foreach ($_POST['contents'] as $languageCode => $contents) {
                $expression = new ToolboxVO_BBS_TopicsTemplateTypeExpression();
                $expression->languageCode = $languageCode;
                $expression->templateType = getTextByTag($templateTypeSet, $languageCode);
                $templateTypes[] = $expression;
            }
			...
			if(!is_array($resultSetFileRecord)){
				...
				$message = $bbsClient->createTopicTemplateType($topic['contents']->id, $templateTypes);
...
//		'debugging' => TRUE,
		...
		'typeCode' => $typeCode,
		'replyId' => $replyId,
        'templateDefs' => $GLOBALS['xoopsModuleConfig']['bbsTemplate'],
