...
function b_filesharing_tophits_show( $options )
{
	global $xoopsDB ;
	...
	$result = $xoopsDB->query( "SELECT l.lid , l.cid , l.title , l.ext , l.res_x , l.res_y , l.submitter , l.status , l.date AS unixtime , l.hits , l.rating , l.votes , l.comments , c.title AS cat_title FROM $table_files l LEFT JOIN $table_cat c ON l.cid=c.cid WHERE l.status>0 AND $whr_cat ORDER BY hits DESC" , $files_num , 0 ) ;
...
function b_filesharing_tophits_edit( $options )
{
	global $xoopsDB ;
