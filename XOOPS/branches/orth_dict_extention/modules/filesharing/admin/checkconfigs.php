...
$rs = $xoopsDB->query( "SELECT COUNT(com_id) FROM $table_comments WHERE com_modid=$filesharing_mid" ) ;
if( ! $rs ) echo "<font color='#FF0000'><b>Error</b></font><br />\n" ;
else echo "<font color='#00FF00'><b>ok</b></font><br />\n" ;
...
// directory
echo _AM_MB_DIRECTORYFORFILES.": ".XOOPS_ROOT_PATH."$filesharing_filespath &nbsp; " ;
if( substr( $filesharing_filespath , -1 ) == '/' ) {
	echo "<font color='#FF0000'><b>"._AM_ERR_LASTCHAR."</b></font><br />\n" ;
} else if( ord( $filesharing_filespath ) != 0x2f ) {
...
if( $filesharing_makethumb ) {
	echo _AM_MB_DIRECTORYFORTHUMBS.": ".XOOPS_ROOT_PATH."$filesharing_thumbspath &nbsp; " ;
	if( substr( $filesharing_thumbspath , -1 ) == '/' ) {
		echo "<font color='#FF0000'><b>"._AM_ERR_LASTCHAR."</b></font><br />\n" ;
	} else if( ord( $filesharing_thumbspath ) != 0x2f ) {
		echo "<font color='#FF0000'><b>"._AM_ERR_FIRSTCHAR."</b></font><br />\n" ;
	...
	} else if( $filesharing_thumbspath == $filesharing_filespath ) {
...
while( list( $lid , $ext ) = $xoopsDB->fetchRow( $rs ) ) {
	...
	if( $filesharing_makethumb && in_array( strtolower( $ext ) , $filesharing_normal_exts ) && ! is_readable( "$thumbs_dir/$lid.$ext" ) ) {
...
// show result
if( $dead_files == 0 ) {
	if( ! $filesharing_makethumb || $dead_thumbs == 0 ) {
		echo "<font color='#00FF00'><b>ok</b></font> &nbsp; <br />\n" ;
	} else {
...
$removed_tmp_num = filesharing_clear_tmp_files( $files_dir ) ;
