...
class PostedNoticeMailDirector {
	...
	function contruct() {
		...
		$this->mBuilder->setSubject($this->mNoticeConfig, $this->mBbsMessages, $this->mBbsOther, $this->mLanguageResource, $this->mXoopsModuleConfig);
...
//		$this->mMailer = getMailer();
	...
	function setSubject($noticeConfig, $bbsMessages, $bbsOther, $languageResource, $xoopsModuleConfig) {
		$tmpl = $languageResource->getSubject(
			$noticeConfig->get('language_code'));
		list($multiTmpl, $singleTmpl) =
			explode('{IF_SINGLE_MSG}', $tmpl, 2);
		$tmpl = (isset($singleTmpl) && count($bbsMessages) === 1)?
			$singleTmpl:$multiTmpl;
		$msg = reset($bbsMessages);
		$subject = $this->makeLine($tmpl, $noticeConfig->get('language_code'), $msg, $bbsOther, $languageResource, $xoopsModuleConfig);
	...
	function setBody($noticeConfig, $bbsMessages, $bbsOther, $languageResource, $xoopsModuleConfig) {
		...
		$tmpl = $languageResource->getBody($noticeConfig->get('language_code'));
		$body = '';
		foreach ($bbsMessages as $message) {
			$body .= $this->makeLine($tmpl, $noticeConfig->get('language_code'), $message, $bbsOther, $languageResource, $xoopsModuleConfig);
			$body .= PHP_EOL;
		}
		$this->mMailer->assign('BODY', $body);
	}

	private function makeLine($tmpl, $language, $message, $bbsOther, $languageResource, $xoopsModuleConfig) {
		...
		$user = $message->text->creator;
		$tmp = str_replace('{AUTHOR}', $user->get('name'), $tmp);
