...
redirect_header(XOOPS_MODULE_URL . "/legacy/admin/index.php?action=PreferenceEdit&confmod_id=$apfilesharing_mid", 1, "Sorry. GO TO Preference!!");
...
if( $action == "insert" ) {
	...
	// newly insert
	$sql = "INSERT INTO $table_folder SET " ;
	...
	if( $cid == intval( $_POST['pid'] ) ) {
		$xoopsDB->query( "UPDATE $table_folder SET pid='0' WHERE cid='$cid'" ) ;
	}
	...
	foreach( $children as $child ) {
		$whr .= "$child," ;
		xoops_notification_deletebyitem( $apfilesharing_mid , 'category' , $child ) ;
	}
	$whr .= "$cid)" ;
	xoops_notification_deletebyitem( $apfilesharing_mid , 'category' , $cid ) ;

	apfilesharing_delete_files( $whr ) ;

	$xoopsDB->query( "DELETE FROM $table_folder WHERE $whr" ) or die( "DB error: DELETE cat table" ) ;
	redirect_header( 'index.php' , 2 , _MD_ALBM_CATDELETED ) ;
	exit ;
...
if( $disp == "edit" && $cid > 0 ) {
	...
	// Editing
	$sql = "SELECT cid,pid,title,imgurl FROM $table_folder WHERE cid='$cid'" ;
	...
	$whr_cid .= "0)" ;
	$rs = $xoopsDB->query( "SELECT * FROM $table_folder WHERE $whr_cid" ) ;
	if( $xoopsDB->fetchArray( $rs ) != false ) {
		$xoopsDB->queryF( "UPDATE $table_folder SET pid='0' WHERE $whr_cid" ) ;
		redirect_header( 'index.php' , 0 , 'A Ghost Category found.' ) ;
		exit ;
	}

	// Waiting Admission
	$ars = $xoopsDB->query( "SELECT COUNT(*) FROM $table_files WHERE status=0" ) ;
	...
	foreach( $cat_tree_array as $cat_node ) {
		...
		$prs = $xoopsDB->query( "SELECT COUNT(*) FROM $table_files WHERE cid='$cid'" ) ;
		list( $files_num ) = $xoopsDB->fetchRow( $prs ) ;
		if( $imgurl && $imgurl != 'http://' ) $imgsrc4show = $myts->htmlSpecialChars( $imgurl ) ;
		else $imgsrc4show = '../images/pixel_trans.gif' ;

		echo "
		<tr>
			<td class='$oddeven' width='4%' style='text-align:right;'>&nbsp;".$myts->htmlSpecialChars($cid)."</td>
			<td class='$oddeven' width='80%'><a href='filemanager.php?cid=$cid'>$prefix&nbsp;".$myts->htmlSpecialChars($title)."</a></td>
			<td class='$oddeven' nowrap='nowrap' align='right'>
				<a href='filemanager.php?cid=$cid'>$files_num</a>
	...
	// Table footer
	//echo "<tr><td colspan='4' align='right' class='foot'><input type='submit' name='batch_update' value='"._AM_CAT_BTN_BATCH."' /></td></tr>" ;
