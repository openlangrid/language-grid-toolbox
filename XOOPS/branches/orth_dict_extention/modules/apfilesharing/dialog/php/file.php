...
if ($cid > 1) {
	...
	if (!$isadmin) {
		...
		$apfilesharing_moduleConfig = $handler->getConfigsByDirname('apfilesharing');
		$apfilesharing_rootedit = $apfilesharing_moduleConfig['apfilesharing_rootedit'];
		if ($apfilesharing_rootedit != '1') {
			$isRootEdit = false;
		}
	}
}
foreach ($a->getChilds() as $f) {
	$response['list'][] = apfilesharingDialog_File_Utils::o2a($f);
}

$response['current'] = $a->getId();
$response['parents'] = array();
foreach ($a->getParents() as $p) {
	$response['parents'][] = apfilesharingDialog_File_Utils::o2a($p);
}
$root = apfilesharingDialog_File_Utils::o2a($a);
if (!$isRootEdit) {
	$root['canEdit'] = false;
}
...
class apfilesharingDialog_File_Utils {
