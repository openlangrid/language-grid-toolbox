...
if( ! defined( 'XOOPS_ROOT_PATH' ) ) {
	if( file_exists( 'mainfile.php' ) ) {
		...
		include( 'modules/apfilesharing/include/read_configs.php' ) ;
	} else {
...
$xoopsTpl->assign('makethumb', $apfilesharing_makethumb);
...
if( sizeof( $cats ) > 0 ) {
	...
	// $whr_ext = "ext IN ('" . implode( "','" , $apfilesharing_normal_exts ) . "')" ;
	$whr_ext = '1' ;
	$select_is_normal = "ext IN ('" . implode( "','" , $apfilesharing_normal_exts ) . "')" ;
	...
	if( $cid > 0 ) {
		...
		if ($total > 0) {
			...
			// use [siteimg] or [img]
			if( empty( $apfilesharing_usesiteimg ) ) {
				...
				$img_tag = 'siteimg' ;
				$url_tag = 'siteurl' ;
				$pdir = substr( $apfilesharing_fpath , 1 ) ;
				$tdir = substr( $apfilesharing_thumbspath , 1 ) ;
			}
			...
			while( list( $lid , $cid , $title , $ext , $submitter , $res_x , $res_y , $is_normal ) = $xoopsDB->fetchRow( $prs ) ) {
				...
				if( ! $is_normal ) {
					...
					if( $apfilesharing_makethumb ) {
						...
						if( $width <= $apfilesharing_thumbsize ) $width_spec = '' ;
					}
				}
...
$xoopsTpl->display( "db:apfilesharing_imagemanager.html" ) ;
exit ;
