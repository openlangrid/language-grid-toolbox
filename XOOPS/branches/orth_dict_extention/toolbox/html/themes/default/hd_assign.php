...
if ($xoopsUser) {
	$this->assign("uname", $xoopsUser->getVar('uname'));
	$xoops_user_groups = is_object($xoopsUser) ? $xoopsUser->getGroups() : array(XOOPS_GROUP_ANONYMOUS);
	if(in_array(XOOPS_GROUP_ADMIN,$xoops_user_groups) || in_array(XOOPS_GROUP_USERS,$xoops_user_groups)){
		if ($xoopsUser->getVar('name')) {
			$this->assign("welcome", sprintf(_US_LOGGINGU, $xoopsUser->getVar('name')));
		} else {
			$this->assign("welcome", sprintf(_US_LOGGINGU, $xoopsUser->getVar('uname')));
		}
	}else{
		$this->assign("welcome", _US_LOGGIN_GUEST);
	}
}
// xoops_breadcrumbs
	...
	foreach( $groups as $group ) {
		...
		foreach( $modules as $module ) {
			if( in_array( $module->getVar('mid') , $read_allowed ) ) {
				...
				if( ! $module->getInfo('sub') ) continue ; 
...
if ( strpos( $this->_tpl_vars['xoops_requesturi'],$this_dirname.'/register.php' ) === 0 ){
	...
	$this->_tpl_vars['xoops_dirname'] = "create_new_account" ;
	...
	$this->_tpl_vars['xoops_dirname'] = "profile" ;
	...
	$this->_tpl_vars['xoops_dirname'] = "lost_pass" ;
