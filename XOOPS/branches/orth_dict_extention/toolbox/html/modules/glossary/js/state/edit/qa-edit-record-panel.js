...
QaEditRecordPanel.prototype.Templates = {
	questionRow : '<tr id="#{id}">'
		...
		+ '<td width="32%" class="#{editableCellClass}" colspan="2" id="#{sourceId}">#{inSourceLanguage}</td>'
		+ '<td width="27%" class="#{editableCellClass}" id="#{targetId}">#{inTargetLanguage}</td>'
		+ '<td width="27%" class="#{editableCellClass}" id="#{categoriesId}">#{categories}</td>'
		+ '<td><button #{attribute} id="#{deleteButtonId}" class="#{deleteClassName}"><span>#{deleteLabel}</span></button></td>'
		+ '</tr>',
	controllerRow : '<tr><td colspan="5">'
