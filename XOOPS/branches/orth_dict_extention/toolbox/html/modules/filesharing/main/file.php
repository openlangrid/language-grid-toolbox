...
$xoopsOption['template_main'] = "filesharing_file.html" ;
...
$file = filesharing_get_array_for_file_assign( $p ) ;
...
$file['width_height'] = '' ;
list( $max_w , $max_h ) = explode( 'x' , $filesharing_middlepixel ) ;
...
$sub_title = preg_replace( "/\'\>/" , "'><img src='$mod_url/images/folder16.gif' alt='' />" , $cattree->getNicePathFromId( $cid , 'title' , "?page=viewcat&num=" . intval( $filesharing_perpage )  ) ) ;
...
if( isset( $_GET['orderby'] ) && isset( $filesharing_orders[ $_GET['orderby'] ] ) ) $orderby = $_GET['orderby'] ;
else if( isset( $filesharing_orders[ $filesharing_defaultorder ] ) ) $orderby = $filesharing_defaultorder ;
else $orderby = 'lidA' ;

// create category navigation
$fullcountresult = $xoopsDB->query( "SELECT lid FROM $table_files WHERE cid=$cid AND status>0 ORDER BY {$filesharing_orders[$orderby][0]}" ) ;
$ids = array() ;
