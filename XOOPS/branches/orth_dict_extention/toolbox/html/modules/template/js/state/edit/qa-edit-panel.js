...
Object.extend(QaEditPanel.prototype, {
	...
	initEventListeners : function() {
		if (!this.observerd) {
			...
			document.observe('source:changed', this.drawWordSets.bind(this));
			this.observerd = true;
		}
...
//		this.addEvent('closeAllAnswersClicked', this.Config.Id.CLOSE_ALL_ANSWERS, 'click', this.Event.closeAllAnswersClicked.bindAsEventListener(this));
		...
		this.addEvent('editWordSetsClicked', this.EDIT_BOUND_WORD_SETS_ID, 'click', this.Event.editWordSetsClicked.bindAsEventListener(this));
	},

	load : function() {
		this.recordsPanel.init();
	...
	createHtml : function() {
		...
		this.resource.wordSetIds.each(function(id){
			if (!Global.WordSets.isEnum(id)) {
				return;
			}
			boundWords.push(Global.WordSets.getName(id, this.sourceLanguage));
		}.bind(this));
		...
		return new Template(this.Templates.base).evaluate({
			...
			boundWordSetsLabel : Global.Text.BOUND_WORD_SETS,
			boundWordSetsId : this.BOUND_WORD_SETS_ID,
			boundWordSets : boundWords.join(', '),
			boundWordSetsAttribute : attribute,
			editBoundWordSetsId : this.EDIT_BOUND_WORD_SETS_ID,
			editBoundWordSets : Global.Text.EDIT_BOUND_WORD_SETS,
	...
	drawWordSets : function() {
		var boundWords = [];
		this.resource.wordSetIds.each(function(id){
			if (!Global.WordSets.isEnum(id)) {
				return;
			}
			boundWords.push(Global.WordSets.getName(id, this.recordsPanel.sourceLanguage));
		}.bind(this));
		$(this.BOUND_WORD_SETS_ID).update(boundWords.join(', '));
	},
	...
	BOUND_WORD_SETS_ID : 'qa-bound-w',
	EDIT_BOUND_WORD_SETS_ID : 'qa-bound-ew'
});
...
QaEditPanel.prototype.Templates = {
	base : '#{languagesLabel}: <span id="#{languagesId}">#{languages}</span> <span #{editLanguagesAttribute} >[<span class="#{clickableClassName}" id="#{editLanguagesId}">#{editLanguages}</span>]</span><br />'
		+ '#{categoriesLabel}: <span id="#{categoriesId}">#{categories}</span> <span #{editCategoriesAttribute} >[<span class="#{clickableClassName}" id="#{editCategoriesId}">#{editCategories}</span>]</span><br />'
		+ '#{boundWordSetsLabel}: <span id="#{boundWordSetsId}">#{boundWordSets}</span> <span #{boundWordSetsAttribute} >[<span class="#{clickableClassName}" id="#{editBoundWordSetsId}">#{editBoundWordSets}</span>]</span>'
		+ '<div class="clearfix">'
...
//			var oldLanguages = popup.resource.languages.clone();
	...
	editWordSetsClicked : function(event) {
