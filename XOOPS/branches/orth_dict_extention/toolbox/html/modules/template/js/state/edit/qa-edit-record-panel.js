...
Object.extend(QaEditRecordPanel.prototype, {
	...
	initialize : function() {
		...
		this.clickEventCache = this.Event.onBlurContents.bindAsEventListener(this);
	},
	...
	initEventListeners : function() {
		...
		this.addEvent('parametersClickEvent', (this.getParametersId()), 'click', this.Event.parametersClicked.bindAsEventListener(this));
		...
		this.addEvent('insertLeftButtonClicked', this.getInsertLeftButtonId(), 'click', this.Event.insertButtonClicked.bindAsEventListener(this));
		this.addEvent('insertRightButtonClicked', this.getInsertRightButtonId(), 'click', this.Event.insertButtonClicked.bindAsEventListener(this));
	},
	...
	save : function() {
		...
		new Ajax.Request(Global.Url.SAVE_RECORD, {
			...
			onSuccess : function(transport) {
				...
				if (this.newFlag) {
					this.newFlag = false;
					this.record.questionId = response.contents.questionId;
				}
				
//				this.record.answers.each(function(answer){
//					if (answer.answerId == null) {
//						answer.answerId = response.contents.answers.pop().answerId;
//					} else {
//						throw $break;
//					}
//				}.bind(this));
...
//		}
	...
	createQuestionRowHtml : function() {
		...
		return new Template(this.Templates.questionRow).evaluate({
			...
			parametersId : this.getParametersId(),
...
//		}.bind(this));
	...
	createSaveCancelRowHtml : function() {
		return new Template(this.Templates.saveCancelRow).evaluate({
			...
			insertLeftId : this.getInsertLeftButtonId(),
			insertRightId : this.getInsertRightButtonId(),
...
//			id : this.getCategoriesId(),
	...
	createParameterHtml : function() {
		...
		this.stateManager.currentRecord.parameterIds.each(function(parameterId, i){
			str = [];
			str.push('[' + i + '] ');
			
			var bool = false;
			for (i = 0; i < this.resource.records.length; i++ ) {
				bool = this.resource.records[i]["parameterIds"].detect(function(id) { return (id == parameterId); });
			}
			
			if (bool) {
				str.push(Global.WordSets.getName(parameterId, this.sourceLanguage));
			} else {
				str.push('text');
			}
			html.push(str.join(''));
		...
		return new Template(Global.Templates.DIV).evaluate({
//			id : this.getParametersId(),
			contents : (html.length == 0) ? Global.Text.BLANK : html.join('<br />')
		});
	},
...
//		}
	...
	getInsertLeftButtonId : function() {
		return this.Config.Id.INSERT_LEFT_PREFIX + this.id;
	},
	
	getInsertRightButtonId : function() {
		return this.Config.Id.INSERT_RIGHT_PREFIX + this.id;
	},
	...
	getParametersId : function() {
		return this.Config.Id.PARAMETERS_PREFIX + this.id;
	},
	...
	isSourceOpen : function() {
		return this.editingId.match(this.Config.Id.SOURCE_PREFIX);
	},
	
	isTargetOpen : function() {
		return this.editingId.match(this.Config.Id.TARGET_PREFIX);
	},
	
	showInsertButton : function() {
		if (this.isSourceOpen()) {
			$(this.getInsertLeftButtonId()).show();
		} else {
			$(this.getInsertRightButtonId()).show();
		}
	},
	
	hideInsertButton : function() {
		$(this.getInsertLeftButtonId()).hide();
		$(this.getInsertRightButtonId()).hide();
	},
	
	isNoBlurEvent : function(event) {
		var id = Event.element(event).id;
		
		return (id == this.getTextareaId());// || (id == this.getInsertLeftButtonId()) || (id == this.getInsertRightButtonId());
	},
	...
	isAllEmpty : function() {
		...
		this.stateManager.languages.each(function(language){
			var expression = this.stateManager.currentRecord.expressions[language];
	...
	createTextarea : function(element) {
		...
		setTimeout(function(){
			Event.observe(document, 'click', this.clickEventCache);
		}.bind(this), 10);
...
QaEditRecordPanel.prototype.Config = {
	Id : {
		...
		PARAMETERS_PREFIX : 'qa-edit-record-edit-parameter-',
		...
		INSERT_LEFT_PREFIX : 'qa-edit-record-insert-left-button-',
		INSERT_RIGHT_PREFIX : 'qa-edit-record-insert-right-button-',
...
QaEditRecordPanel.prototype.Event = {
	...
	onBlurContents : function(event) {
		...
		if (this.isNoBlurEvent(event)) {
			return;
		}
		
		Event.stopObserving(document, 'click', this.clickEventCache);
	...
	parametersClicked : function(event) {
		if (!!Global.recordPanel && Global.recordPanel != this) {
			if (!confirm(Global.Text.CONFIRM_DISCARD_CHANGES)) {
				return;
			}
		...
		var element = Event.element(event);
		var popup = new QaEditSaveParameterPopupPanel();
		popup.parent = this;
		popup.resource = this.resource;
		popup.language = this.sourceLanguage;
		popup.record = this.stateManager.currentRecord;
		popup.parameterIds = this.stateManager.currentRecord.parameterIds.clone();
		popup.recordPanel = this;
		popup.stateManager = this.stateManager;
//		popup.editingId = this.editingId;
//		popup.leftFlag = this.isSourceOpen();
	...
	insertButtonClicked : function(event) {
		...
		popup.onHidePanel = function() {
			this.draw();
			this.createTextarea($(popup.editingId));
			setTimeout(function(){
				Event.observe(document, 'click', this.clickEventCache);
			}.bind(this), 10);
		}.bind(this);
...
QaEditRecordPanel.prototype.Templates = {
	questionRow : '<tr id="#{id}">'
		...
		+ '<td width="23%" class="#{editableCellClass}" id="#{parametersId}">#{parameters}</td>'
