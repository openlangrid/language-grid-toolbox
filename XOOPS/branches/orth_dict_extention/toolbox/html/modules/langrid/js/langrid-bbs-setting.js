...
var BBSAddPathWorkspace = Class.create();

Object.extend(BBSAddPathWorkspace.prototype, AddPathWorkspace.prototype);
Object.extend(BBSAddPathWorkspace.prototype, {
	_showPanel: function(setting,listArea) {
		var idx = this.pathPanelArray.length;
		var pathPanel = new BBSPathPanel();
	...
	_loadSetting: function() {
		...
		new Ajax.Request('./ajax/load-bbs-setting.php', {
			method: 'post',
			parameters: hash,
	...
	onDictionarySaveHandler:function(){
		...
		var hash = $H(postObj).toQueryString();
		new Ajax.Request('./ajax/save-bbs-dict.php', {
			method: 'post',
			parameters: hash,
			controller: controller,
			onSuccess: function(httpObj) {
