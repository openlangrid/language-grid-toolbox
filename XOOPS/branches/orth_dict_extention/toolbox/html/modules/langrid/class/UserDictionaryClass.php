...
//$time_start = microtime(true);
		if (file_exists(XOOPS_ROOT_PATH.'/modules/dictionary/php/lib/user-dictionary-controller.php')) {
			...
			$langKeys = array_flip($langSets);
			if (!$langKeys) { return null; }


			// 
			if ($sourceLang == $targetLang) {
				if (!array_key_exists($sourceLang, $langKeys) || !array_key_exists($targetLang.'-normalized', $langKeys)) { return null; }
				$srcKey = $langKeys[$sourceLang];
				$tgtKey = $langKeys[$targetLang.'-normalized'];
			} else {
				if (!array_key_exists($sourceLang, $langKeys) || !array_key_exists($targetLang, $langKeys)) { return null; } 	
				$srcKey = $langKeys[$sourceLang];
