...
class BBS_CategoriesBodyObject extends XoopsSimpleObject {

	function BBS_CategoriesBodyObject() {
...
class BBS_CategoriesBodyHandler extends Toolbox_CompositeKeyGenericHandler {

	var $mTable = "";
	var $mModName = "";
	var $mPrimary = "cat_id";
	var $mClass = "BBS_CategoriesBodyObject";
	var $mPrimaryAry = array('cat_id', 'language_code');
	
	public function __construct(&$db,$moduleName) {
		parent::__construct($db);
		$this->mTable = $db->prefix($moduleName."_categories_body");
	...
	function &getObjects($criteria = null, $limit = null, $start = null, $id_as_key = false){
		$ret = array();
		$categoryTable = $this->db->prefix($this->mModName.'_categories');
		
		$sql  = "SELECT * FROM `" . $this->mTable . '`';
		$sql .= " WHERE cat_id in (select cat_id from ".$categoryTable." where delete_flag = 0) ";
