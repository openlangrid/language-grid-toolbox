...
eval( ' function xoops_module_uninstall_'.$mydirname.'( $module ) { return task_onuninstall_base( $module , "'.$mydirname.'" ) ; } ' ) ;


if( ! function_exists( 'task_onuninstall_base' ) ) {

function task_onuninstall_base( $module , $mydirname )
{
	...
	if( defined( 'XOOPS_CUBE_LEGACY' ) ) {
		...
		$root->mDelegateManager->add( 'Legacy.Admin.Event.ModuleUninstall.' . ucfirst($mydirname) . '.Success' , 'task_message_append_onuninstall' ) ;
		$ret = array() ;
	} else {
...
function task_message_append_onuninstall( &$module_obj , &$log )
{
