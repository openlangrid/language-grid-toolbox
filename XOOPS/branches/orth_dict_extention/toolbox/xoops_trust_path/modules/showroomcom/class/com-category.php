...
class Com_Category {
	...
	private function __construct($category) {
		...
		$this -> id = $category->mVars['cat_id']['value'];
	}
	
	public function getTitleForLanguage($languageCode) {
		foreach($this -> category -> body as $body) {
	...
	public function getOriginalLanguage() {
		return $this -> category -> mVars['post_original_language']['value'];
	}
	...
	static public function findByCategoryId($categoryId) {
		...
		return new Com_Category($bbs -> getCategory($categoryId));
	}
}
?>
