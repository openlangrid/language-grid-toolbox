...
function communication_get_forums_can_read( $mydirname )
{
	global $xoopsUser ;
...
function communication_get_categories_can_read( $mydirname )
{
	global $xoopsUser ;
...
function communication_get_submenu( $mydirname )
{
	...
	$whr_read4cat = '`cat_id` IN (' . implode( "," , communication_get_categories_can_read( $mydirname ) ) . ')' ;
	$whr_read4forum = '`forum_id` IN (' . implode( "," , communication_get_forums_can_read( $mydirname ) ) . ')' ;
	...
	// restruct categories
	$submenus_cache[$mydirname] = array_merge( communication_restruct_categories( $categories , 0 ) ) ;
	return $submenus_cache[$mydirname] ;
}


function communication_restruct_categories( $categories , $parent )
{
	$ret = array() ;
	foreach( $categories as $cat_id => $category ) {
		if( $category['pid'] == $parent ) {
			...
			$ret[] = array(
				'name' => $category['name'] ,
				'url' => $category['url'] ,
				'sub' => array_merge( $category['sub'] , communication_restruct_categories( $categories , $cat_id ) ) ,
			) ;
		}
	}
...
function communication_common_is_necessary_antispam( $user , $mod_config )
{
...
function &communication_common_get_antispam_object( $mod_config )
{
...
function communication_common_unhtmlspecialchars( $text )
{
	return strtr( $text , array_flip( get_html_translation_table( HTML_SPECIALCHARS , ENT_QUOTES ) ) + array( '&#039;' => "'" ) ) ;
}


function communication_common_simple_request( $params )
{
	$myts =& MyTextSanitizer::getInstance() ;
	$requests = array() ;
	$whrs = array() ;
...
function communication_common_utf8_encode_recursive( &$data )
{
	if( is_array( $data ) ) {
		foreach( array_keys( $data ) as $key ) {
			communication_common_utf8_encode_recursive( $data[ $key ] ) ;
		}
