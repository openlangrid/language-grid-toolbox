...
TranslationPathPanel.prototype = {
	...
	_createViewPanel: function(data) {
		...
		for(var i=0;i<=2;i++){
			dict_btn[i] = document.createElement('a');
			var tx = document.createTextNode('View dictionary(ies)');
			dict_btn[i].appendChild(tx);
		...
		tr[1].appendChild(this.makeTdObj("&nbsp;"));
		tr[1].appendChild(this.makeTdObj("",this.makeServiceViewObj(service1,dict_btn[0],data.dict_flags[0])));
		...
		if(lang3 != ""){
			...
			tr[0].appendChild(this.makeTdObj("",this.makeDivObj(flow,"allow-cell")));
			tr[1].appendChild(this.makeTdObj("&nbsp;"));
			tr[1].appendChild(this.makeTdObj("",this.makeServiceViewObj(service2,dict_btn[1],data.dict_flags[1])));
			if(lang4 != ""){
				...
				tr[1].appendChild(this.makeTdObj("&nbsp;"));
				tr[1].appendChild(this.makeTdObj("",this.makeServiceViewObj(service3,dict_btn[2],data.dict_flags[2])));
			}else{
		...
		ViewPanel.appendChild(this._createEditArea());
	...
	_createEditArea: function() {
		...
		Element.addClassName(edit_area, 'edit-area');
		...
		Event.observe(edit_btn, 'click', this._onEditButtonClicked.bind(this));
		wrapper.appendChild(edit_btn);

		wrapper.appendChild(document.createTextNode(']'));
		edit_area.appendChild(wrapper);
...
//		postObj['data'] = save_data;
	...
	makeServiceViewObj: function(service,dictbtn,custflg){
		var ret = document.createElement('div');
		Element.addClassName(ret, 'service-cell');

		var svc = document.createElement('span');
		Element.addClassName(svc, 'server-name');
		svc.innerHTML = service;

		var dic = document.createElement('div');
		dic.appendChild(document.createTextNode('['));
		dic.appendChild(dictbtn);
		dic.appendChild(document.createTextNode(']'));
		if (custflg == '2') {
			dic.appendChild(document.createTextNode('*'));
		}
	...
	_buildeData: function() {
		try {
			...
			if(!this.isSupportedAnalyzer(data.morph_analyzer1,data.lang1)){
				data.morph_analyzer1 = getDefaultMorphologicalAnalyzer(data.lang1);
			}
			if(!this.isSupportedAnalyzer(data.morph_analyzer2,data.lang2)){
				data.morph_analyzer2 = getDefaultMorphologicalAnalyzer(data.lang2);
			}
			if(!this.isSupportedAnalyzer(data.morph_analyzer3,data.lang3)){
				data.morph_analyzer3 = getDefaultMorphologicalAnalyzer(data.lang3);
			}
			if(!this.isSupportedAnalyzer(data.morph_analyzer4,data.lang4)){
				data.morph_analyzer4 = getDefaultMorphologicalAnalyzer(data.lang4);
			}
	...
	isSupportedAnalyzer: function(service_id,lang){
		...
		this._langridServiceInformations['_analyzerServicesArray'].each(function(service, index){
			if(service.service_id == service_id){
				...
				if (service.service_type == 'ANALYZER' && supported_langs.indexOf(lang) > -1) {
					ret = true;
					throw $break;
				}
			}
