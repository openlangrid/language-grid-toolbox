...
//		$objects =& $this->m_DefaultDictionarySettingDao->getObjects($mCriteriaComp);
    ...
    function loadDefaultDictionaryBinds($settingId) {
        return $this->m_DefaultDictionaryBindDao->queryBySettingId($settingId);
    }
	...
	function removeDefaultDictionaryBind($settingId) {
		...
		if ($DefaultSetting != null) {
            $binds = $this->m_DefaultDictionaryBindDao->queryBySettingId($settingId);
	...
	function addDefaultDictionarySetting($userId, $setId) {
        ...
        $DefaultDictionarySetting->set('create_date', time());

		if (($settingId = $this->m_DefaultDictionarySettingDao->insert($DefaultDictionarySetting->getVars(), true))) {
            $DefaultDictionarySetting->set('setting_id', $settingId);
	...
	function addDefaultDictionaryBind($settingId, $type, $bind) {
		$bindId = 1;
		$currentBinds = $this->m_DefaultDictionaryBindDao->queryBySettingId($settingId);
		if ($currentBinds != null) {
			$bindId = count($currentBinds) + 1;
		}
        ...
        $DefaultDictionaryBind->set('create_date', time());
        $DefaultDictionaryBind->set('delete_flag', '0');

		if ($this->m_DefaultDictionaryBindDao->insert($DefaultDictionaryBind->getVars(), true)) {
	...
	function updateDefaultDict4TranslationPath($userId,$setId){
        ...
        $binds = $this->m_DefaultDictionaryBindDao->queryBySettingId($DefaultDict->get('setting_id'));

		if(is_array($binds)){
			foreach($binds as $bind){
				$btype = $bind->getBindType();
				if($btype != 9){
					$dicts[$btype][] = $bind->getBindValue();
				}
			}
		}
					...
					foreach($execs as $exec){
						...
						if(intval($dflg) == 1 || intval($dflg) == 0){
							...
							if(count($dicts[1]) == 0 && count($dicts[2]) == 0 && count($dicts[3]) == 0){
								...
								$this->m_ExecDao->update($exec, true);
							}else{
